...
def check_and_switch_python():
    ...
    if sys.executable != CORRECT_PYTHON:
        ...
        try:
            ...
            result = subprocess.run([CORRECT_PYTHON, __file__], 
                                  env=env, 
...
def tokenize(text: str) -> set:
    tokens = text.split()
    stopwords=["a", "an", "the", "of", "in", "on", "by", "to", "for", "as", "was", "were", "is", "are", "within"]
...
def fact_coverage(file_path_article: str, file_path_factbank: str):
    ...
    for core_fact_idx in range(len(core_facts_embeddings)):
        for sentence_idx in range(len(sentences_embeddings)):
            ...
            print(f"Sentence:{sentences_sets[sentence_idx]}, Core Fact:{core_facts_sets[core_fact_idx]}, Similarity:{similarity}")
            if similarity > 0.75:
                score += 1
            for i in range(5):
                print()
