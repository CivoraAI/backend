...
class TrainDatasetForEmbedding(Dataset):
    def __init__(
        ...
        if os.path.isdir(args.train_data):
            ...
            self.dataset = datasets.load_dataset(
                "json", data_files=args.train_data, split="train", cache_dir=args.cache_path
            )
        ...
        self.prefix_ids = self.tokenizer(
            self.prefix, truncation=True, max_length=self.max_length, return_tensors=None
        )["input_ids"]
    ...
    def __getitem__(self, item):
        ...
        input_ids = self.tokenizer(
            ...
            max_length=self.max_length
            - len(prefix_ids)
            - len(suffix_ids_summarize)
            - len(suffix_ids_predict),
            padding=False,
            return_tensors=None,
            add_special_tokens=False,
        )
        result = dict()
        result["input_ids"] = (
            prefix_ids + input_ids["input_ids"] + suffix_ids_summarize + suffix_ids_predict
        )
        result["attention_mask"] = [1] * len(result["input_ids"])
        result["labels"] = (
            [-100] * len(prefix_ids)
            + result["input_ids"][
                len(prefix_ids) : len(result["input_ids"])
                - len(suffix_ids_summarize)
                - len(suffix_ids_predict)
            ]
        ...
        if self.remove_stop_words:
            ...
            oup_summarize = " ".join(
                [word for word in oup_summarize.split() if word.lower() not in self.stop_words]
            )

            oup_predict = re.sub(r"[\W_]+", " ", oup_predict)
            oup_predict = " ".join(
                [word for word in oup_predict.split() if word.lower() not in self.stop_words]
            )
