...
class TrainDatasetForEmbedding(Dataset):
    def __init__(self, args: DataArguments, tokenizer: PreTrainedTokenizer):
        ...
        self.query_max_len = (
            self.args.query_max_len - len(self.prefix_ids) - len(self.suffix_query_ids)
        )
        self.passage_max_len = (
            self.args.passage_max_len - len(self.prefix_ids) - len(self.suffix_passage_ids)
        )
    ...
    def __getitem__(self, item) -> Tuple[BatchEncoding, List[BatchEncoding]]:
        ...
        query_inputs["input_ids"] = (
            self.prefix_ids + query_inputs["input_ids"] + self.suffix_query_ids
        )
        ...
        if len(self.dataset[item]["neg"]) < self.args.train_group_size - 1:
            ...
            num = math.ceil(
                (self.args.train_group_size - 1) / len(list(set(self.dataset[item]["neg"])))
            )
        ...
        for passage in passages:
            ...
            passage_inputs["input_ids"] = (
                self.prefix_ids + passage_inputs["input_ids"] + self.suffix_passage_ids
            )
