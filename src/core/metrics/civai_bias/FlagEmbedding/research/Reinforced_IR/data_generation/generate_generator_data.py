...
def main(opt):
    ...
    if model_type == "llm":
        ...
        llm = GPTAgent(model_name=generate_model_path, api_key=api_key, base_url=base_url)
    ...
    for file_path in os.listdir(dataset_path):
        ...
        for idx in range(dpo_num - len(queries_corpus_list)):
            prompts = [
                get_additional_info_generation_prompt(file_path, qc["query"])
                for qc in queries_corpus
            ]
            ...
            for i, output in enumerate(outputs):
                ...
                queries_corpus_list[-1][i]["new_query"] = (
                    "Generate the topic about this passage: " + output
                )

            with open(answers_output_dir, "w") as f:
                json.dump(queries_corpus_list, f)

    retrieval_model = FlagModel(
    ...
    for file_path in os.listdir(dataset_path):
        ...
        llm_train_data = generate_llm_dpo_train_data(
            queries_corpus_list,
            "answer",
            "passage",
            retrieval_model,
