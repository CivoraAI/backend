...
def main(opt):
    ...
    for file_path in os.listdir(dataset_path):
        ...
        if not os.path.exists(queries_output_dir):
            prompts = [
                get_query_generation_prompt(file_path, c[:8000], use_examples=True) for c in corpus
            ]
            ...
            qualities_prompts = [
                get_quality_control_prompt(file_path, q, c)
                for (q, c) in zip(generated_queries, corpus)
            ]
            ...
            for i in range(len(generated_qualities)):
                if "1" in generated_qualities[i]:
                    queries_corpus.append({"query": generated_queries[i], "passage": corpus[i]})
