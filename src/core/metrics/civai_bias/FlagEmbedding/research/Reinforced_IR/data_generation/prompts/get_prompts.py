...
CLAIM_QUERY_LENGTH_LIST = ["10 to 15 words"] * 5 + ["at least 15 words"] * 2
...
def get_additional_info_generation_train_prompt(
    dataset_name: str, query: str, reference: str
) -> str:
    task = TASK_DICT[dataset_name]
    query_type = QUERY_TYPE_DICT[dataset_name]
    answer_type = ANSWER_TYPE_DICT[dataset_name]
...
Your output:"""
    ...
    prompt = prompt_template.format(
        task=task, query_type=query_type, query=query, answer_type=answer_type, reference=reference
    )
    return prompt
