...
def generate_bge_train_data(
    ...
    queries_emb = retrieval_model.encode_queries(
    ...
    answers_emb = retrieval_model.encode_corpus(
        answers, batch_size=batch_size, max_length=max_length
    )
    if emb_save_path is not None:
        if os.path.exists(emb_save_path):
            if ignore_prefix:
                doc_emb = np.vstack(
                    (
                        retrieval_model.encode_corpus(
                            corpus[: len(queries_emb)], batch_size=batch_size, max_length=max_length
                        ),
            ...
            doc_emb = retrieval_model.encode_corpus(
    ...
    for i in trange(len(queries), desc="generate train set"):
        ...
        train_data.append(
            {"query": query, "answer": answer, "pos": pos, "neg": negs, "neg_answer": negs_answer}
        )
...
def generate_llm_dpo_train_data(
    ...
    raw_queries_emb = retrieval_model.encode_queries(
        raw_queries, batch_size=batch_size, max_length=max_length
    )
    ...
    for queries in queries_list:
        ...
        queries_emb = (
            raw_queries_emb * 0.8
            + retrieval_model.encode_queries(queries, batch_size=batch_size, max_length=max_length)
            * 0.2
        )
    ...
    for i in range(len(all_scores_list[0])):
        ...
        if use_rule1:
            if max_score > raw_score and (max_score - raw_score * 0.8) * threshold >= (
                min_score - raw_score * 0.8
            ):
                # print('use')
                tmp = {
