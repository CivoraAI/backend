...
class Args:
    generate_model_path: str = field(
        default="Meta-Llama-3-8B", metadata={"help": "Generate model path"}
    )
    generate_model_lora_path: str = field(default=None, metadata={"help": "Generate model path"})
    temperature: float = field(default=0.8, metadata={"help": "Temperature for generation"})
    gpu_memory_utilization: float = field(default=0.8, metadata={"help": "GPU memory utilization"})
    top_p: float = field(default=1.0, metadata={"help": "Top p for generation"})
    max_tokens: int = field(default=300, metadata={"help": "Max tokens for generation"})
    model_type: str = field(default="llm_instruct", metadata={"help": "LLM model type"})
    input_dir: str = field(default=None, metadata={"help": "Input directory", "required": True})
    output_dir: str = field(default=None, metadata={"help": "Output directory", "required": True})
    num_gpus: int = field(default=8, metadata={"help": "Number of GPUs"})
    rm_tmp: bool = field(default=True, metadata={"help": "Remove temporary files"})
    start_gpu: int = field(default=0)
