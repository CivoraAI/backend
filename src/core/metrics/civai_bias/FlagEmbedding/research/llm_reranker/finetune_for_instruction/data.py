...
class TrainDatasetForReranker(Dataset):
    ...
    def __getitem__(self, item) -> List[BatchEncoding]:
        ...
        positive_inputs = (
            self.tokenizer(prompt, return_tensors=None, add_special_tokens=False)["input_ids"]
            + self.tokenizer("Yes", return_tensors=None, add_special_tokens=False)["input_ids"]
        )
        ...
        for i, passage in enumerate(passages):
            ...
            passage_inputs["labels"] = [-100] * (
                len(passage_inputs["input_ids"]) - 1
            ) + passage_inputs["labels"][(len(passage_inputs["input_ids"]) - 1) :]
            (
