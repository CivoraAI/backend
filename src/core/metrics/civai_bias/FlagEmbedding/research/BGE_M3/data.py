...
class SameDatasetTrainDataset(Dataset):
            ...
            for file in os.listdir(data_dir):
                ...
                if len(temp_dataset) == 0:
                    ...
                    small_batch_size = min(
                        small_batch_size,
                        self.get_file_batch_size(
                            file, batch_size, train_group_size=args.train_group_size
                        ),
                    )
                else:
                    if (
                        args.max_example_num_per_dataset is not None
                        and len(temp_dataset) > args.max_example_num_per_dataset
                    ):
    ...
    def print_batch_size(self, batch_size: int, train_group_size: int):
        ...
        batch_size_dict = {
            k: self.get_file_batch_size(f"len-{k}.jsonl", batch_size, train_group_size)
            for k in length_list
        }
        batch_size_list = [f"{length}: {batch_size_dict[length]}" for length in length_list]
        print("=========================")
