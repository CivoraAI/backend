...
class BGEM3Model(nn.Module):
    ...
    def forward(
        ...
        if self.enable_sub_batch:
            q_dense_vecs, q_sparse_vecs, q_colbert_vecs = self.encode(
                query, sub_batch_size=self.compute_sub_batch_size(query)
            )
            p_dense_vecs, p_sparse_vecs, p_colbert_vecs = self.encode(
                passage, sub_batch_size=self.compute_sub_batch_size(passage)
            )
        else:
            ...
            if teacher_scores is not None:
                ...
                if self.negatives_cross_device:
                    ...
                    loss = self.distill_loss(
                        cross_teacher_targets, cross_dense_scores, group_size=group_size
                    )
                else:
                ...
                if self.unified_finetuning:
                    ...
                    sparse_loss = self.distill_loss(
                        teacher_targets, sparse_scores, group_size=group_size
                    )

                    colbert_scores = self.colbert_score(
                        q_colbert_vecs, p_colbert_vecs, q_mask=query["attention_mask"]
                    )  # B, B * N
                    colbert_loss = self.distill_loss(
                        teacher_targets, colbert_scores, group_size=group_size
                    )

                    ensemble_loss = self.distill_loss(
                        teacher_targets,
                ...
                idxs = torch.arange(
                    q_dense_vecs.size(0), device=q_dense_vecs.device, dtype=torch.long
                )
                ...
                if self.negatives_cross_device:
                    ...
                    cross_idxs = torch.arange(
                        cross_q_dense_vecs.size(0),
                        device=cross_q_dense_vecs.device,
                        dtype=torch.long,
                    )

                    cross_targets = cross_idxs * (
                        cross_p_dense_vecs.size(0) // cross_q_dense_vecs.size(0)
                    )
                    cross_dense_scores = self.dense_score(cross_q_dense_vecs, cross_p_dense_vecs)
                ...
                if self.unified_finetuning:
                    ...
                    ensemble_loss = self.compute_loss(
                        dense_scores + 0.3 * sparse_scores + colbert_scores, targets
                    )
                    loss = (loss + ensemble_loss + 0.1 * sparse_loss + colbert_loss) / 4

            if (
                self.use_self_distill
                and self.step > self.self_distill_start_step
                and self.unified_finetuning
            ):
                ...
                loss += (
                    ensemble_distill_dense_loss
                    + 0.1 * ensemble_distill_sparse_loss
                    + ensemble_distill_colbert_loss
                ) / 3
                loss = loss / 2
            self.step += 1
        else:
    ...
    def load_pooler(self, model_dir):
        colbert_state_dict = torch.load(
            os.path.join(model_dir, "colbert_linear.pt"), map_location="cpu"
        )
        sparse_state_dict = torch.load(
            os.path.join(model_dir, "sparse_linear.pt"), map_location="cpu"
        )
...
class BGEM3ForInference(BGEM3Model):
    ...
    def forward(
        ...
        assert (
            return_dense or return_sparse or return_colbert
        ), "Must choose one or more from `return_colbert`, `return_sparse`, `return_dense` to set `True`!"
        ...
        if return_sparse:
            sparse_vecs = self.sparse_embedding(
                last_hidden_state, text_input["input_ids"], return_embedding=return_sparse_embedding
            )
        ...
        if self.normlized:
            ...
            if "colbert_vecs" in output:
                output["colbert_vecs"] = torch.nn.functional.normalize(
                    output["colbert_vecs"], dim=-1
                )
