...
peft_config = LoraConfig(
    task_type=TaskType.CAUSAL_LM, inference_mode=False, r=16, lora_alpha=32, lora_dropout=0.0
)
...
def get_context_emb(conv, model, img_list, answer_prompt=None, print_res=False):
    ...
    with torch.no_grad():
        seg_tokens = [
            model.llama_tokenizer(seg, return_tensors="pt", add_special_tokens=i == 0)
            .to("cuda:0")
            .input_ids
        ...
        seg_embs = [
            model.llama_model.base_model.model.model.embed_tokens(seg_t) for seg_t in seg_tokens
        ]
...
def get_sinusoid_encoding_table(
    n_position=784, d_hid=1024, cur_frame=8, ckpt_num_frame=4, pre_n_position=784
):
    ...
    sinusoid_table = torch.tensor(sinusoid_table, dtype=torch.float, requires_grad=False).unsqueeze(
        0
    )
...
class MLVU(Dataset):
        ...
        for k, v in data_list.items():
            ...
            for data in json_data:
                self.data_list.append(
                    {"task_type": k, "prefix": v[1], "data_type": v[2], "data": data}
                )

        self.decord_method = {"video": self.read_video}
    ...
    def __str__(self):
        ...
        for k, v in len_list.items():
            ...
            total += option_list[k]
            res += (
                f"{v} for {k} ({option_list[k]} options => {len_list[k]/option_list[k]*100:.2f}%)\n"
            )
...
new_pos_emb = get_sinusoid_encoding_table(
    n_position=(resolution // 16) ** 2 * num_frame, cur_frame=num_frame
)
...
def infer_mvbench(
    ...
    chat = EasyDict(
        {"system": system, "roles": ("Human", "Assistant"), "messages": [], "sep": "###"}
    )
...
with open(f"{save_path}.json", "w") as f:
    json.dump({"acc_dict": acc_dict, "res_list": res_list}, f)
