...
from .utils import (
    ...
    merge_param,
    compute_weights,
    get_model_param_dirs,
    merge_param_by_layer,
)


def save_ckpt_for_sentence_transformers(
    ckpt_dir, pooling_mode: str = "cls", normalized: bool = True
):
    ...
    if normalized:
        ...
        model = SentenceTransformer(
            modules=[word_embedding_model, pooling_model, normalized_layer], device="cpu"
        )
    else:
...
def mix_models(
    model_names_or_paths: List[str], model_type: str, weights: List[float], output_path: str = None
):
    """_summary_
    ...
    if output_path is not None:
        ...
        if model_type == "encoder":
            print(
                f"Transform the model to the format of 'sentence_transformers' (pooling_method='cls', normalized=True)"
            )
            save_ckpt_for_sentence_transformers(ckpt_dir=output_path)
    return model


def mix_models_with_data(
...
def mix_models_by_layers(
    ...
    with init_empty_weights():
        if model_type == "decoder":
            meta_model = AutoModelForCausalLM.from_pretrained(
                model_names_or_paths[0], trust_remote_code=True
            )
    ...
    model = load_checkpoint_and_dispatch(
        meta_model, checkpoint=temp_file_path, device_map=device_map
    )
    model.tie_weights()
