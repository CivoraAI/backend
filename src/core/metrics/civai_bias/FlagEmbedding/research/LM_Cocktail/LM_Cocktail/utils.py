...
def load_llm(model_name: str, trust_remote_code: bool):
    model = AutoModelForCausalLM.from_pretrained(
        model_name, trust_remote_code=trust_remote_code, device_map={"": "cpu"}
    )
    return model
...
def merge_param_by_layer(model_param_dirs: List[str], weights: List[float]):
    ...
    for param_file in tqdm(model_params, desc="Merging models"):
        ...
        for w, model_dir in tqdm(
            zip(weights, model_param_dirs),
            total=len(weights),
            desc=f"Processing {param_name}",
            leave=False,
        ):
...
def compute_weights(
    ...
    if model_type == "decoder":
        ...
        input_data = preprocess_data_for_seq2seq(
            example_data=example_data,
            tokenizer=tokenizer,
            device=device,
            batch_size=batch_size,
...
def preprocess_data_for_seq2seq(
    example_data, tokenizer, device, batch_size: int = 2, max_input_length: int = 512
):  # Added Reimer
    batch_data = []
...
def preprocess_data_for_embedder(
    example_data,
    tokenizer,
    device,
    batch_size: int = 64,
    ...
    if len(quries) > 0:
        q_tokens = tokenizer(
            quries, padding=True, truncation=True, max_length=max_input_length, return_tensors="pt"
        )
        p_tokens = tokenizer(
            passages,
            padding=True,
...
def preprocess_data_for_llm(
    example_data, tokenizer, device, batch_size: int = 2, max_input_length: int = 2048
):
    batch_input_ids = []
    batch_labels = []
