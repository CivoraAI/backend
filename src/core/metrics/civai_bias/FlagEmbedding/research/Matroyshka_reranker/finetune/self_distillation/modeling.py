        ...
        if self.training:
            if isinstance(ranker_logits, List):
                ...
                for idx, logits in enumerate(ranker_logits[::-1]):
                    ...
                    if "distill" in self.train_method:
                        student_scores = logits.view(self.train_batch_size, -1)
                        ...
                        if "teacher" in self.train_method:
                            loss += -torch.mean(
                                torch.sum(
                                    torch.log_softmax(student_scores, dim=-1) * teacher_targets,
                                    dim=-1,
                                )
                            )
                            ...
                            teacher_targets_new = torch.softmax(
                                student_scores_full.detach(), dim=-1
                            )
                            continue
                ...
                if self.train_method == "distill":
                    ...
                    teacher_scores = teacher_scores.view(self.train_batch_size, -1)
                    teacher_targets = torch.softmax(teacher_scores.detach(), dim=-1)
                    student_scores = ranker_logits.view(self.train_batch_size, -1)
                    loss += -torch.mean(
                        torch.sum(
