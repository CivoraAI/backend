...
class TrainDatasetForReranker(Dataset):
    def __init__(self, args: DataArguments, tokenizer: PreTrainedTokenizer):
        if os.path.exists(args.train_data[-1]):
            ...
            for file in data_path:
                try:
                    temp_dataset = datasets.load_dataset(
                        "json", data_files=file, split="train", cache_dir=args.cache_path
                    )
            ...
            self.dataset = datasets.load_dataset(
                args.train_data[-1], split="train", cache_dir=args.cache_path
            )
    ...
    def __getitem__(self, item) -> tuple[List[BatchEncoding], List[int], List[int], List[int]]:
        ...
        for neg in negs:
            try:
                scores.append(
                    self.dataset[item]["neg_scores"][self.dataset[item]["neg"].index(neg)]
                )
            except:
                scores.append(1)
        ...
        positive_inputs = self.tokenizer(prompt, return_tensors=None, add_special_tokens=False)[
            "input_ids"
        ]
        ...
        for i, passage in enumerate(passages):
            ...
            if (
                ...
                query_inputs_length.append(
                    len([self.tokenizer.bos_token_id] + query_inputs["input_ids"] + self.sep_inputs)
                )
            else:
                item = self.tokenizer.prepare_for_model(
            ...
            (
                passage_inputs.pop("token_type_ids")
                if "token_type_ids" in passage_inputs.keys()
                else None
            )
