...
class SameDatasetTrainDataset(Dataset):
    ...
    def __init__(
        self, args: DataArguments, batch_size, seed, tokenizer, process_index=0, num_processes=1
    ):
        train_datasets = []
        each_data_inxs = []
        batch_size_inxs = []
            ...
            if os.path.exists(os.path.join(args.save_disk_path, FLAG_LOG_NAME)):
                ...
                if args.num_shards <= 0:
                    self.dataset.save_to_disk(
                        args.save_disk_path, max_shard_size=args.save_max_shard_size
                    )
                else:
                    self.dataset.save_to_disk(args.save_disk_path, num_shards=args.num_shards)
                with open(
                    os.path.join(args.save_disk_path, FLAG_LOG_NAME), "w", encoding="utf-8"
                ) as f:
                    log_info = {
                        "train_data": args.train_data,
    ...
    def refresh_epoch(self):
        print(
            f"---------------------------*Rank {self.process_index}: refresh data---------------------------"
        )
        ...
        for dataset_inx in self.datasets_inxs:  # 按洗了小数据集的顺序，加载所有的数据集
            self.deterministic_generator.shuffle(
                self.each_data_inxs[dataset_inx]
            )  # 洗了小数据集内数据的顺序
            cur_batch_size = (
                self.batch_size_inxs[dataset_inx] * self.num_processes
            )  # 总batch_size，小batch_size * num_processes
            for start_index in range(0, len(self.each_data_inxs[dataset_inx]), cur_batch_size):
                ...
                if start_index + cur_batch_size > len(self.each_data_inxs[dataset_inx]):
                    ...
                    break
                batch_datas.append(
                    self.each_data_inxs[dataset_inx][start_index : start_index + cur_batch_size]
                )
    ...
    def __getitem__(self, idx):
        ...
        self.step += 1
        queries_inputs, passages_inputs, messages, scores = self.create_batch_data(
            batch_raw_data=batch_data
        )
    ...
    def create_batch_data(self, batch_raw_data):
        ...
        if "symmetric" in finetune_type and (
        ...
        for i in range(len(batch_raw_data["query"])):
            ...
            if self.args.retrieval_use_examples or (
                "clustering" in batch_raw_data["type"][i]
                or "sts" in batch_raw_data["type"][i]
                or "class" in batch_raw_data["type"][i]
            ):
                if "clustering" in batch_raw_data["type"][i]:
                    icl_pairs.append(
                        (
                            self.tokenizer.decode(
                                self.tokenizer(queries[-1], add_special_tokens=False)["input_ids"][
                                    : self.args.example_query_max_len
                                ]
                            ),
                            self.tokenizer.decode(
                                self.tokenizer(
                                    batch_raw_data["category"][i], add_special_tokens=False
                    ...
                    icl_pairs.append(
                        (
                            ...
                            self.tokenizer.decode(
                                self.tokenizer(pos, add_special_tokens=False)["input_ids"][
            ...
            if "sts" in batch_raw_data["type"][i] or "clustering" in batch_raw_data["type"][i]:
                tmp_passages = [
                    get_query_prompt(p, batch_raw_data["prompt"][i], self.args.use_special_tokens)
