...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            position_ids_l = torch.arange(seq_length, dtype=torch.long, device=query.device).view(
                -1, 1
            )
            position_ids_r = torch.arange(seq_length, dtype=torch.long, device=query.device).view(
                1, -1
            )
