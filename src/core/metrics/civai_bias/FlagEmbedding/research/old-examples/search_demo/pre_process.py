...
class EmbDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        attention_mask = batch_dict["attention_mask"][0].tolist() + [0] * (
            512 - len(batch_dict["attention_mask"][0])
        )
        token_type_ids = batch_dict["token_type_ids"][0].tolist() + [0] * (
            512 - len(batch_dict["token_type_ids"][0])
        )
...
def inference(json_path, emb_path, model_path):
    ...
    for step, data in enumerate(tqdm(loader, total=len(loader))):
        ...
        with torch.no_grad():
            ...
            batch_vecs = (
                torch.nn.functional.normalize(batch_vecs, p=2, dim=-1).detach().cpu().numpy()
            )
...
if __name__ == "__main__":
    ...
    inference(
        os.path.join(collection_path, "documents.json"),
