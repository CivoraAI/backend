...
class Visualized_BGE(nn.Module):
    def __init__(
        ...
        self.model_visual, self.preprocess_train, self.preprocess_val = (
            create_eva_vision_and_transforms(model_name_eva, force_custom_clip=True)
        )
    ...
    def encode_mm(self, images: torch.Tensor, texts):
        ...
        prompt_position_ids = torch.arange(prompt_start, prompt_start + prompt_len - 1).to(
            device=device
        )
        ...
        head_mask = [None] * self.depth
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(
            prom_img_attention_mask, prom_img_input_shape
        ).to(self.dtype)
        ...
        prompt_img_reps = self.sentence_embedding(
            sequence_output, prom_img_attention_mask
        )  # tensor: reps with pooling
        if self.normlized:
    ...
    def img_token_embedding(self, images):
        if images is None:
            return None
        img_token_emb = self.model_visual.encode_image(
            images, normalize=False
        )  # return_all_features=True, [B, Patch_num, C]
