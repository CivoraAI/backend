...
class LLM:
    def __init__(
        ...
        if model_type == "open-source":
            ...
            self.client = OpenAI(
                api_key=os.getenv("OPENAI_API_KEY"), base_url=os.getenv("OPENAI_BASE_URL", None)
            )
        else:
    ...
    def split_text(self, text: str, anchor_points: Tuple[float, float] = (0.4, 0.7)):
        ...
        return self.tokenizer.decode(token_ids[:split_index]), self.tokenizer.decode(
...
if __name__ == "__main__":
    llm = LLM(model="gpt-4o-mini-2024-07-18", model_type="openai")
