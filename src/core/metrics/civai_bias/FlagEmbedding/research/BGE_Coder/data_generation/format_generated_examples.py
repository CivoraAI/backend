...
def format_generated_examples(file_path: str, save_path: str, task_type: TaskType):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f.readlines():
            ...
            data_list.append({"input": _input, "output": _output})
            ...
            if task_type == TaskType.code_translation_retrieval:
                for code_language_pair in CODE_TRANSLATION_RETRIEVAL_PAIRS:
                    ...
                    file_path = os.path.join(
                        ...
                        language.name,
                        task_type.name,
                        f"{language.name}-{code_language.name}-to-{tgt_code_language.name}-triplets.jsonl",
                    )
                    save_path = os.path.join(
                        formatted_examples_dir,
                        language.name,
                        task_type.name,
                        f"{code_language.name}-to-{tgt_code_language.name}_sample_examples.json",
                    )
                ...
                file_path = os.path.join(
                    ...
                    language.name,
                    task_type.name,
                    f"{language.name}-sql-triplets.jsonl",
                )
                save_path = os.path.join(
                    formatted_examples_dir,
                    language.name,
                    task_type.name,
                    "sql_sample_examples.json",
                )
                ...
                for code_language in CodeLanguage:
                    ...
                    file_path = os.path.join(
                        ...
                        language.name,
                        task_type.name,
                        f"{language.name}-{code_language.name}-triplets.jsonl",
                    )
                    save_path = os.path.join(
                        formatted_examples_dir,
                        language.name,
                        task_type.name,
                        f"{code_language.name}_sample_examples.json",
                    )
