    ...
    def build_windows(self):
        ...
        for task in self.tasks:
            ...
            for i, window in enumerate(merged_code_windows):
                ...
                if window["metadata"][0]["fpath_tuple"] != fpath_tuple and " ".join(
                    list(window["metadata"][0]["fpath_tuple"])
                ) != " ".join(list(fpath_tuple)):
                    continue
                # print(1)
                if any(
                    [
...
def download_data(directory: str = "repoeval"):
    ...
    if not os.path.exists(repos_function_dir):
        ...
        repos_function_url = (
            "https://github.com/microsoft/CodeT/raw/main/RepoCoder/repositories/function_level.zip"
        )
    ...
    for i, window in enumerate(windows):
        ...
        if i in ground_truth_indices:
            for query_id in ground_truth_indices[i]:
                ...
                query_id2gt[query_id].append(
                    {"title": corpus_id.replace("_", "/"), "text": window["context"]}
                )
...
def main():
    ...
    if args.split == "function":
        ...
        for task in data:
            ...
            task["metadata"]["task_id"] = (
                task["metadata"]["task_id"].replace("--", "_").replace("idx", str(repo2idx[repo]))
            )
    ...
    for repo in REPOs:
        ...
        results_file = os.path.join(
            args.results_dir, f"repoeval-{args.split}-{repo}-{args.context_length}-gt.jsonl"
        )
        ...
        results_file = os.path.join(
            args.results_dir, f"repoeval-{args.split}-{repo}-{args.context_length}-infile.jsonl"
        )
        with open(results_file, "w") as fw:
            for ex in gt_data:
