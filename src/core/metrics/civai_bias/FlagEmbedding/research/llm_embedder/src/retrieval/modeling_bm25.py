...
class BM25Retriever:
    ...
    def _prepare_collection(
        self, corpus: datasets.Dataset, collection_dir, max_docs_per_file=1000000
    ):
    ...
    def _prepare_query(
        self, eval_data: Union[str, datasets.Dataset], query_dir: str, max_queries_per_file=10000
    ):
        clear_dir(query_dir)
        ...
        if isinstance(eval_data, str):
            ...
            raise ValueError(
                f"Expected eval_data to be instance of str or datasets.Dataset, got {type(eval_data)}!"
            )
            ...
            for idx, (qid, query) in enumerate(zip(query_ids, queries)):
                if idx % max_queries_per_file == 0:
                    if idx > 0:
                        g.close()
                    g = open(
                        os.path.join(query_dir, f"queries.{str(idx // max_queries_per_file)}.tsv"),
                        "w",
                    )
    ...
    def search(
        self,
        eval_data: Union[str, datasets.Dataset],
        ...
        **kwds,
        ...
        if output_dir is None and not hasattr(self, "output_dir"):
            raise ValueError(
                f"Make sure there is an index by either calling .index() or specifying an existing index with index_dir=xxx!"
            )
...
class NaiveBM25Retriever:
    ...
    def index(
        self,
        corpus: List[Union[str, List[int]]],
        verbose: bool = False,
    ...
    def search(
        self,
        queries: Union[str, List[int], List[str], List[List[int]]],
        hits: int = 100,
        k1: Optional[float] = None,
        b: Optional[float] = None,
        verbose: bool = False,
        ...
        for i, query in enumerate(iterator):
            ...
            for token in query:
                ...
                tfs = np.array(
                    [self.tfs[candidate][token] for candidate in candidates], dtype=np.float32
                )
                df = self.dfs[token]
                idf = np.log((self.N - df + 0.5) / (df + 0.5) + 1)

                candidate_scores = (
                    idf * (k1 + 1) * tfs / (tfs + k1 * (1 - b + b * self.doc_length[candidates]))
                )
