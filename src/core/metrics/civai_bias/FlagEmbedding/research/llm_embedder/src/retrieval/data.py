...
class RetrievalDataset:
        ...
        def _process(
            ...
            **kwds,
            ...
            if teacher_scores is not None:
                assert len(teacher_scores) == len(pos) + len(
                    neg
                ), f"Found incompatible teacher_score size ({len(teacher_scores)}) and positive size ({len(pos)}) negative size ({len(neg)})"
            ...
            if select_positive == "random":
                assert (
                    pos_num > 0
                ), f"Select positive strategy 'random' is only available when there is a given positive!"
                ...
                assert (
                    pos_num > 0
                ), f"Select positive strategy 'teacher-pos' is only available when there are teacher_scores and positives!"
                ...
                assert (
                    pos_num > 0
                ), f"Select positive strategy 'first' is only available when there is a given positive!"
                pos_idx = 0
                pos = pos[0]
            ...
            if teacher_scores is not None and select_negative == "teacher-":
                neg_indices = [
                    i
                    for i, _ in sorted(enumerate(neg_scores), key=lambda x: x[1])[
                        : train_group_size - 1
                    ]
                ]
                ...
                neg_indices = [
                    i
                    for i, _ in sorted(enumerate(neg_scores), key=lambda x: x[1], reverse=True)[
                        : train_group_size - 1
                    ]
                ]
    ...
    def prepare_train_dataset(
        ...
        for path in data_file:
            ...
            temp_dataset = temp_dataset.map(
                process_fn, batched=True, num_proc=32, remove_columns=temp_dataset.column_names
            )
            # limit sample number
            ...
            if dataset_name in dataset_indices_range:
                ...
                dataset_indices_range[f"{dataset_name}_{dataset_dup[dataset_name]}"] = (
                    offset,
    ...
    def prepare_eval_dataset(
        data_file=None, cache_dir=None, instruction=None, eval_method="retrieve"
    ):
        if data_file is None:
            return None
        ...
        def _process(
            ...
            key_index: Optional[List[int]] = None,
            pos: Optional[List[Union[int, str]]] = None,
            neg: Optional[List[str]] = None,
            pos_index: Optional[List[int]] = None,
            neg_index: Optional[List[int]] = None,
            _index=None,
            **kwds,
        ):
            if instruction is not None:
...
class SameDatasetTrainDataset(torch.utils.data.Dataset):
    ...
    def create_epoch(self):
        ...
        for k, x in self.dataset_indices_range.items():
            ...
            batches = dataset_range.reshape(
                num_batches, self.batch_size * self.num_processes
            ).tolist()
    ...
    def __getitem__(self, idx):
        if self.organize_method == "random":
            ...
            dataset_name = self.deterministic_generator.choice(
                range(len(self.sample_range)), size=1, p=sample_prob
            )[0]
            sample_range = self.sample_range[dataset_name]

            batch_indices = self.nondeterministic_generator.choice(
                sample_range, size=self.batch_size, replace=False
            )
            ...
            raise NotImplementedError(
                f"Organize method {self.organize_method} is not implemented for SameTaskTrainDataset!"
            )
    ...
    def __call__(self, batch_elem):
        ...
        for k, v in first_elem.items():
            ...
            if k == "query":
                ...
                assert all(
                    v == batch_value[0] for v in batch_value
                ), f"Make sure all samples are of the same task in a batch!"
...
TASK_CONFIG = {
    ...
    "e5": {"instruction": defaultdict(lambda: {"query": "query: ", "key": "passage: "})},
    "instructor": {
        "instruction": {
            "qa": {
