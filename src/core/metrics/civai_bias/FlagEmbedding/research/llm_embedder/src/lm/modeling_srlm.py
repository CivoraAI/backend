...
class SelfRetrievalLM(LM):
    def __init__(
        ...
        **kwds,
        ...
        assert (
            context_window_size % chunk_size == 0
        ), f"Make sure the context_window_size ({context_window_size}) is divisible by chunk_size ({chunk_size})!"
    ...
    def _get_retrieved_chunks(self, value_chunks, retrieved_indices):
        ...
        indices = indices[..., None].expand(
            batch_size * chunk_batch_size, 2 * self.key_num, self.chunk_size
        )  # batch_size * chunk_batch_size, 2k, chunk_size
        # Slice out the retrieved chunk and its continuation from the corpus
        retrieved_chunks = (
            value_chunks.repeat_interleave(chunk_batch_size, dim=0)
            .gather(dim=1, index=indices)
            .view(indices.shape[0], self.key_num, 2 * self.chunk_size)
    ...
    def forward_with_history_retrieval(
        self, query: np.ndarray, history: np.ndarray, answer: np.ndarray, history_mask: torch.Tensor
    ):
        ...
        if self.retrieval_method == "no":
            ...
            history_embedding = self.retriever.encode(keys.tolist()).unflatten(
                0, (batch_size, history_size)
            )  # B * N, D
            ...
            assert (
                self.key_num == 1 and batch_size == 1
            ), f"Retrieval_method 'oracle' is only available when k == 1 and batch_size == 1!"
            ...
            for hist_idx in range(history_size):
                ...
                arange = torch.arange(labels.shape[1] - 1, -1, -1, device=self.device).expand(
                    labels.shape
                )
    ...
    def forward_with_chunk_retrieval(self, input_ids, attention_mask, labels):
        ...
        if inputs_length % self.chunk_size != 0:
            ...
            input_ids = torch.cat(
                [
                    input_ids.new_zeros(batch_size, pad_length) + self.tokenizer.pad_token_id,
                    input_ids,
                ],
                dim=-1,
            )
            attention_mask = torch.cat(
                [attention_mask.new_zeros(batch_size, pad_length), attention_mask], dim=-1
            )
        ...
        ).all(), f"Make sure all targets in the batch starts from the same token index!"
        target_start_index = target_start_index[0].item()
        assert (
            target_start_index % self.chunk_size == 0
        ), f"Make sure the target_length ({inputs_length} - {target_start_index} = {inputs_length - target_start_index}) is divisible by chunk_size ({self.chunk_size})!"
        ...
        if self.integrate_method == "replace":
            assert n_window_chunk >= (
                n_target_chunk + 1 + 2 * self.key_num
            ), f"Make sure there are at least k * 2 + 1 + n_target_chunk = {self.key_num * 2 + 1 + n_target_chunk} chunks (found {context_window_size} / {self.chunk_size} = {n_window_chunk}) that can be replaced with retrieved contents!"
            ...
            labels = torch.cat(
                [labels.new_zeros(batch_size, 2 * self.key_num * self.chunk_size) - 100, labels],
                dim=-1,
            )
            labels_mask_indices_offset = 2 * self.key_num * self.chunk_size
        else:
            raise NotImplementedError(
                f"Integration strategy {self.integrate_method} not implemented!"
            )
        ...
        arange = (
            torch.arange(self.chunk_size, device=input_ids.device)
            + target_chunk_start_idx * self.chunk_size
        )
        labels_mask_indices = bias[:, None] + arange[None, :]
        labels_mask_indices = (
            labels_mask_indices.view(n_query_chunk, self.chunk_size) + labels_mask_indices_offset
        )

        if self.retrieval_method == "dense":
            # Encode queries and keys
            queries = self.tokenizer.batch_decode(
                query_chunks.flatten(0, 1), skip_special_tokens=True
            )
            ...
            for i in range(n_query_chunk):
                ...
                sub_labels_mask.scatter_(
                    dim=-1, index=labels_mask_indices[None, i].expand(batch_size, -1), value=False
                )
        ...
        for i in range(0, n_query_chunk, self.chunk_batch_size):
            ...
            if self.retrieval_method == "dense":
                ...
                rel_score = torch.einsum(
                    "bid,bjd->bij", query_embedding, key_embeddings
                )  # batch_size, chunk_batch_size, n_key_chunk
                retrieved_indices = rel_score.topk(self.key_num, dim=-1)[1].flatten(
                    0, 1
                )  # batch_size * chunk_batch_size, k

            elif self.retrieval_method == "random":
                retrieved_indices = torch.randint(
                    0,
                    n_key_chunk,
                    (batch_size * chunk_batch_size, self.key_num),
            ...
            # batch_size * chunk_batch_size, k * 2 * chunk_size
            retrieved_chunks, retrieved_indices = self._get_retrieved_chunks(
                value_chunks, retrieved_indices
            )

            # Each sub-batch has its own retrieved contexts
            sub_input_ids = torch.cat(
                [retrieved_chunks, fixed_context.repeat_interleave(chunk_batch_size, dim=0)], dim=-1
            )
            sub_attention_mask = (sub_input_ids != self.tokenizer.pad_token_id).long()
            ...
            sub_labels = labels.repeat_interleave(
                chunk_batch_size, dim=0
            )  # batch_size * chunk_batch_size, n_window_chunk * self.chunk_size
            sub_labels_mask = torch.ones_like(sub_labels, dtype=torch.bool)
            # NOTE: only compute loss for this sub-batch
            sub_labels_mask.scatter_(
                dim=-1,
                index=labels_mask_indices[None, i:j].expand(batch_size, -1, -1).flatten(0, 1),
                value=False,
            )
            sub_labels = sub_labels.masked_fill(sub_labels_mask, -100)
            ...
            if self.debug_retrieval:
                ...
                chunks = retrieved_chunks.view(
                    batch_size, chunk_batch_size, self.key_num, 2 * self.chunk_size
                )
            ...
            outputs = self.model(
                input_ids=sub_input_ids, attention_mask=sub_attention_mask, labels=sub_labels
            )
            if (sub_labels == -100).all():
                # NOTE: in this case, the model will return nan. We correct its behavior by returning 0
                loss = 0
