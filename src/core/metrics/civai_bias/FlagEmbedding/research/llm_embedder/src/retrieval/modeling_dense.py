...
class DenseRetriever(torch.nn.Module):
    def __init__(
        ...
        **kwds,
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            query_encoder, cache_dir=cache_dir, truncation_side=truncation_side
        )
        ...
        if tie_encoders:
            encoder = AutoModel.from_pretrained(
                query_encoder, cache_dir=cache_dir, torch_dtype=dtype
            ).to(self.device)
            ...
            self.key_encoder = AutoModel.from_pretrained(
                key_encoder, cache_dir=cache_dir, torch_dtype=dtype
            ).to(self.device)
    ...
    def _post_init(self):
        ...
        if "dense" in self.pooling_method:
            self.dense_pooler = torch.nn.Linear(self.ndim, self.ndim, bias=False).to(
                device=self.device, dtype=self.query_encoder.dtype
            )
            try:
                state_dict = torch.load(
                    os.path.join(self.query_encoder_name, "dense_pooler.bin"),
                    map_location=self.device,
                )
                self.dense_pooler.load_state_dict(state_dict)
            except:
                logger.warning(
                    f"Could not find dense pooler weight in {self.query_encoder_name}, initialize it randomly!"
                )
    ...
    def _gather_tensors(self, local_tensor):
        ...
        if local_tensor is None:
            return None
        all_tensors = [
            torch.empty_like(local_tensor) for _ in range(self.accelerator.num_processes)
        ]
    ...
    def _save_to_memmap(
        self, path: str, shape: tuple, array: np.ndarray, start: int, batch_size: int = 100000
    ):
        """
        ...
        if self.accelerator.process_index == 0:
            if os.path.exists(path):
                ...
                makedirs(path)
            memmap = np.memmap(path, shape=shape, mode="w+", dtype=array.dtype)
            del memmap

        self.accelerator.wait_for_everyone()

        logger.info(f"saving array at {path}...")
        memmap = np.memmap(path, shape=shape, mode="r+", dtype=array.dtype)
    ...
    def _prepare(self, inputs: Union[str, List[str], Mapping], field="key"):
        ...
        if isinstance(inputs, str) or (isinstance(inputs, list) and isinstance(inputs[0], str)):
            ...
            raise ValueError(
                f"Expected inputs of type str, list[str], or dict, got {type(inputs)}!"
            )
        return inputs

    def _pool(self, embeddings, attention_mask):
        if "mean" in self.pooling_method:
            embeddings = embeddings.masked_fill(~attention_mask[..., None].bool(), 0.0)
    ...
    def encode(
        self, inputs: Union[str, List[str], Mapping], field: str = "key", with_grad: bool = False
    ):
    ...
    def _compute_loss(self, query_embedding, key_embedding, teacher_scores):
        if teacher_scores is not None and self.distill_weight > 0:
            ...
            if self.stable_distill:
                ...
                student_key = key_embedding.view(
                    student_query.shape[0], -1, student_query.shape[-1]
                )  # B, N, D
                student_scores = student_query.matmul(student_key.transpose(-1, -2)).squeeze(
                    1
                )  # B, N
                if self.dense_metric == "cos":
    ...
    def _refresh_config(self, task):
        if hasattr(self, "train_config"):
            ...
            self.contrastive_weight = train_config.get(
                "contrastive_weight", self._contrastive_weight
            )
            self.distill_weight = train_config.get("distill_weight", self._distill_weight)
            self.teacher_temperature = train_config.get(
                "teacher_temperature", self._teacher_temperature
            )
            self.student_temperature = train_config.get(
                "student_temperature", self._student_temperature
            )
    ...
    def index(
        ...
        **kwds,
        ...
        index_path = os.path.join(
            output_dir,
            f"{embedding_name}.{index_factory}.{self.accelerator.process_index}-{self.accelerator.num_processes}.faiss",
        )

        sampler = Sequential_Sampler(
            len(corpus), self.accelerator.num_processes, self.accelerator.process_index
        )
        self._corpus_offset = sampler.start

        if load_encode:
            encoded_corpus = np.memmap(encode_path, mode="r", dtype=np.float32).reshape(
                len(corpus), self.ndim
    ...
    def search(self, inputs: Union[str, List[str], Mapping], hits: int = 10, **kwds):
        ...
        if self.accelerator.num_processes > 1:
            ...
            gathered_batch_scores = (
                self.accelerator.gather(batch_scores)
                .unflatten(0, (self.accelerator.num_processes, -1))
                .tolist()
            )
            gathered_batch_indices = (
                self.accelerator.gather(batch_indices)
                .unflatten(0, (self.accelerator.num_processes, -1))
                .tolist()
            )
        else:
        ...
        for batch_idx in range(batch_scores.shape[0]):
            score = sum(
                [
                    gathered_batch_scores[i][batch_idx]
            ...
            indice = sum(
                [
                    gathered_batch_indices[i][batch_idx]
                    for i in range(self.accelerator.num_processes)
                ],
                [],
            )
            ...
            pair = sorted(
                zip(score, indice), key=lambda x: x[0] if x[1] >= 0 else -float("inf"), reverse=True
            )[:hits]
    ...
    def rerank(self, query, key, key_mask=None, **kwds):
        ...
        key_embeddings = key_embeddings.unflatten(
            0, (query_embeddings.shape[0], -1)
        )  # batch_size, key_num, embedding_dim
    ...
    def save_pretrained(self, output_dir: str, *args, **kwargs):
        if self.tie_encoders:
            self.tokenizer.save_pretrained(os.path.join(output_dir, "encoder"))
            self.query_encoder.save_pretrained(os.path.join(output_dir, "encoder"))
            if hasattr(self, "dense_pooler"):
                torch.save(
                    self.dense_pooler.state_dict(),
                    os.path.join(output_dir, "encoder", "dense_pooler.bin"),
                )

        else:
            self.tokenizer.save_pretrained(os.path.join(output_dir, "query_encoder"))
            self.query_encoder.save_pretrained(os.path.join(output_dir, "query_encoder"))
            self.key_tokenizer.save_pretrained(os.path.join(output_dir, "key_encoder"))
            self.key_encoder.save_pretrained(os.path.join(output_dir, "key_encoder"))
            if hasattr(self, "dense_pooler"):
                torch.save(
                    self.dense_pooler.state_dict(),
                    os.path.join(output_dir, "query_encoder", "dense_pooler.bin"),
                )
