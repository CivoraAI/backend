...
def convert_ours_ckpt_to_sentence_transformer(
    src_dir, dest_dir, pooling_method: List[str] = ["cls"], dense_metric: str = "cos"
):
    assert os.path.exists(src_dir), f"Make sure the encoder path {src_dir} is valid on disk!"
    assert (
        "decoder" not in pooling_method
    ), f"Pooling method 'decode' cannot be saved as sentence_transformers because it uses the decoder stack to produce sentence embedding."
    if dest_dir is None:
        dest_dir = src_dir

    print(
        f"loading model from {src_dir} and saving the sentence_transformer model at {dest_dir}..."
    )
...
@dataclass
class Args:
    encoder: Optional[str] = field(default=None, metadata={"help": "Path to the encoder model."})
    output_dir: Optional[str] = field(
        default=None, metadata={"help": "Path to the output sentence_transformer model."}
    )
    ...
    model_cache_dir: Optional[str] = field(
        default=None, metadata={"help": "Cache folder for huggingface transformers."}
    )

    def __post_init__(self):
        convert_ours_ckpt_to_sentence_transformer(
            self.encoder, self.output_dir, self.pooling_method, self.dense_metric
        )
