...
class ScoreArgs(LMArgs):
    ...
    context_max_length: int = field(default=1024, metadata={"help": "Max length for lm."})
    key_max_length: int = field(default=512, metadata={"help": "Max length for key."})
    lm_batch_size: int = field(
        default=4,
        metadata={"help": "Evaluation json file."},
    )
    save_name: str = field(default="llama2-7b-chat", metadata={"help": "Name of the scored file."})
    load_score: bool = field(default=False, metadata={"help": "Load score from temperary file?"})
    ...
    def _process(
        ...
        **kwds,
        ...
        if history is not None:
            assert (
                task == "chat"
            ), f"Found history={history} is not None but task={task} is not 'chat'!"
            keys = history
        else:
            keys = pos + neg
        for i, key in enumerate(keys):
            ...
            if task == "lrlm":
                ...
                key = tokenizer.decode(
                    tokenizer.encode(
                        key, add_special_tokens=False, max_length=key_max_length, truncation=True
                    )
                )
                ...
                # find answer length
                answer_seq = tokenizer.encode(
                    "Answer: " + answer.lstrip(" "), add_special_tokens=False
                )
                answer_length = len(answer_seq) - len(
                ...
                labels = [
                    x if inputs["attention_mask"][i] == 1 else -100 for i, x in enumerate(labels)
                ]
                labels[:-answer_length] = [-100] * (len(labels) - answer_length)
                inputs["labels"] = labels
...
def collate_scores(eval_data, save_name):
    ...
    def collate(query_ids, scores):
        ...
        data_save_path = os.path.join(
            eval_data_folder, f"{eval_data_name}.scored.{save_name}" + eval_data_ext
        )
        ...
        try:
            ...
            with open(eval_data) as f, open(data_save_path, "w") as g:
                for query_id, score in tqdm(zip(query_ids, scores)):
                    if (query_id != prev_query_id) and (prev_query_id is not None):
                        ...
                        assert (
                            prev_query_id == sample["query_id"]
                        ), f"Found incompatible query_id from data ({sample['query_id']}) and from eval_preds ({prev_query_id})"
                        if "history" in sample:
                            assert len(sample["history"]) == len(
                                teacher_scores
                            ), f"Found incompatible key number from data ({len(sample['history'])}) and from eval_preds ({len(teacher_scores)})"
                        else:
                            assert len(sample["pos"] + sample["neg"]) == len(
                                teacher_scores
                            ), f"Found incompatible key number from data ({len(sample['pos'] + sample['neg'])}) and from eval_preds ({len(teacher_scores)})"
            ...
            logger.error(
                f"Error when trying to save to json file. Save scores to {save_path} instead!"
            )
...
def main():
    ...
    accelerator = Accelerator(
        cpu=args.cpu, kwargs_handlers=[InitProcessGroupKwargs(timeout=timedelta(seconds=100000))]
    )
