...
class PopQAArgs(LMArgs, RetrievalArgs):
    ...
    eval_data: str = field(
        default="llm-embedder:qa/popqa/test.json", metadata={"help": "Path to the test file."}
    )
    ...
    corpus: str = field(
        default="llm-embedder:qa/nq/corpus.json", metadata={"help": "Corpus path for retrieval."}
    )
    ...
    key_max_length: int = field(
        default=128, metadata={"help": "How many tokens at maximum in a key."}
    )
    ...
    log_path: str = field(
        default="data/results/popqa/popqa.log", metadata={"help": "Path to the file for logging."}
    )


@dataclass
class GenerationArgs(GenerationArgs):
    max_new_tokens: int = field(default=16, metadata={"help": "Maximum new tokens to generate."})
    eos_token_id: int = 13
...
def process_popqa(
    ...
    if few_shot > 0:
        ...
        assert few_shot // (
            len(PROPID_2_TEMPLATE) - 1
        ), f"Make sure the number of few shot examples is a multiple of the template number!"
        train_dataset = datasets.load_dataset(
            "json", data_files=train_data, cache_dir=cache_dir, split="train"
        )
    ...
    def _process(query, query_id, prop_id, key=None, _index=None, **kwds):
        ...
        train_samples_max_length = (
            context_max_length
            - len(
                tokenizer.encode(
                    "\n\n" if len(knowledge) else "" + _prepare_sample(query),
            ...
            for query_id, generation in zip(*eval_preds):
                ...
                for answer in answers:
                    ...
                    if (
                        ...
                        or answer.lower() in generation
                        or answer.capitalize() in generation
                    ):
                        correct = True
                        break
...
def main():
    ...
    if accelerator.process_index == 0:
        ...
        result_path = os.path.join(
            args.output_dir, args.model_name_or_path.strip(os.sep).replace(os.sep, "--") + ".json"
        )
