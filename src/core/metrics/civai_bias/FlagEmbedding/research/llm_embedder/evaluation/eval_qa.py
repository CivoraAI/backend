...
class QAArgs(LMArgs, RetrievalArgs):
    ...
    eval_data: str = field(
        default="llm-embedder:qa/nq/test.json", metadata={"help": "Path to the test file."}
    )
    ...
    query_max_length: int = field(
        default=32, metadata={"help": "How many tokens at maximum in a query."}
    )
    ...
    log_path: str = field(
        default="data/results/qa/qa.log", metadata={"help": "Path to the file for logging."}
    )


@dataclass
class GenerationArgs(GenerationArgs):
    max_new_tokens: int = field(default=32, metadata={"help": "Maximum new tokens to generate."})
    eos_token_id: int = 13
    ...
    def compute_metric(eval_preds):
        ...
        with open(save_path, "w") as f:
            for query_id, generation in zip(*eval_preds):
                ...
                em = max(
                    normalize_text(generation) == normalize_text(answer)
