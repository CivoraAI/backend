...
class ICLArgs(LMArgs, RetrievalArgs):
    ...
    task_names: Optional[List[str]] = field(
        default=None, metadata={"help": "List of tasks to evaluate."}
    )
    load_prev_result: bool = field(
        default=False, metadata={"help": "Load existing results in output_dir?"}
    )
    ...
    corpus: str = field(
        default="llm-embedder:icl/icl/corpus.json", metadata={"help": "Corpus path for retrieval."}
    )
    ...
    log_path: str = field(
        default="data/results/icl/icl.log", metadata={"help": "Path to the file for logging."}
    )


@dataclass
class GenerationArgs(GenerationArgs):
    max_new_tokens: int = field(default=64, metadata={"help": "Maximum new tokens to generate."})
...
def load_test_data(
    ...
    for i, e in enumerate(tqdm(dataset, desc="Organizing Data")):
        ...
        if knn_inxs is not None:
            if filter_diff_task:
                ...
                candidates = knn_inxs[i][:example_num]
                few_shot = [
                    remove_double_space(passage_dataset[int(x)]["contents"]) for x in candidates
                ]
        else:
            few_shot = []
    ...
    if same_task_random:
        ...
        for task_name, task_examples in tqdm(
            task_data.items(), desc="Collecting Same-Task-Random Examples"
        ):
            ...
            for i, _ in enumerate(task_examples):
                ...
                few_shot = [
                    remove_double_space(content)
                    for content in passage_dataset[random.sample(task_indices, example_num)][
                        "contents"
                    ]
                ]
...
def main():
    ...
    args.output_dir = os.path.join(
        args.output_dir, args.model_name_or_path.strip(os.sep).replace(os.sep, "--")
    )

    task_data = load_test_data(
        preds,
        test_data=args.eval_data,
        corpus_data=args.corpus,
        example_num=args.few_shot,
        same_task_random=args.retrieval_method == "same-task-random",
    )
