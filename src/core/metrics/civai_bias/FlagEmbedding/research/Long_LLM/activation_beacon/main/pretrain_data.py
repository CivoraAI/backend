...
class Args(ModelArgs):
    config: str = field(
        ...
        metadata={
            "help": "Configuration json path for standard pretraining (concatenating multiple documents to form instances of equal lengths)."
        },
    )
    ...
    num_token: List[str] = field(
        ...
        metadata={
            "help": "How many tokens to use for a specified length? (T/t for trillion, B/b for billion, M/m for million)"
        },
    )
    add_bos: bool = field(default=True, metadata={"help": "Add bos at the end of each document?"})
    add_eos: bool = field(default=True, metadata={"help": "Add eos at the end of each document?"})
...
def prepare_pretrain_data(
    ...
    assert os.path.isdir(
        data_files
    ), f"Make sure the data_files parameter is a directory containing the pretraining data json files! Found {data_files}."
    ...
    for file_name in os.listdir(data_files):
        ...
        num_token_this_dataset = {
            k: math.ceil(v * mix_portion) for k, v in length_2_num_token.items()
        }
        num_instances_this_dataset = {
            k: math.ceil(v / k) for k, v in num_token_this_dataset.items()
        }
        ...
        dataset = datasets.load_dataset(
            "json", data_files=file_path, split="train", cache_dir=cache_dir
        )
        dataset = dataset.map(
            _process,
            batched=True,
        ...
        for x in dataset:
            ...
            if add_bos:
                ...
                ), f"Make sure the bos_token_id exists when enable add_eos."
                sample_input_ids = [tokenizer.bos_token_id] + sample_input_ids
            if add_eos:
                assert (
                    tokenizer.eos_token_id is not None
                ), f"Make sure the eos_token_id exists when enable add_eos."
            ...
            if len(input_ids) >= max_length:
                ...
                while cursor + max_length <= len(input_ids):
                    ...
                    max_length_candidates = [
                        k for k, v in num_instances_this_dataset.items() if v > 0
                    ]
        ...
        if len(max_length_candidates) > 0:
            logger.warning(
                f"There are not enough data ! The remainings are {num_instances_this_dataset} instances for {dataset_name} dataset. Consider increase the corresponding data in {data_files}."
            )
...
if __name__ == "__main__":
    ...
    accelerator = Accelerator(
        cpu=args.cpu, kwargs_handlers=[InitProcessGroupKwargs(timeout=timedelta(days=10))]
    )
    ...
    if accelerator.process_index == 0:
        ...
        if args.add_eos:
            assert (
                tokenizer.eos_token_id is not None
            ), "Make sure the eos_token_id is not None when enabling add_eos!"
