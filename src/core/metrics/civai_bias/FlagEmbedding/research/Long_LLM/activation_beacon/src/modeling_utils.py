...
class RotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=32768, base=10000, device=None):
        ...
        self.base = base
        inv_freq = 1.0 / (
            self.base ** (torch.arange(0, self.dim, 2, dtype=torch.float32).to(device) / self.dim)
        )
...
class LinearScalingRotaryEmbedding(RotaryEmbedding):
    ...
    def __init__(
        self, dim, max_position_embeddings=32768, base=10000, device=None, scaling_factor=1.0
    ):
...
class YarnRotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        self._set_cos_sin_cache(
            seq_len=math.ceil(max_position_embeddings * scaling_factor),
            device=device,
            dtype=torch.get_default_dtype(),
        )
    ...
    def _get_factor(self):
        ...
        fast_dim = (
            ...
            / 2
            * (
                math.log(self.max_position_embeddings / (2 * math.pi * self.beta_fast))
                / math.log(self.base)
            )
        )
        ...
        slow_dim = (
            ...
            / 2
            * (
                math.log(self.max_position_embeddings / (2 * math.pi * self.beta_slow))
                / math.log(self.base)
            )
        )
    ...
    def __init__(
        ...
        for freq in inv_freq:
            ...
            if wavelen < high_freq_wavelen:
                ...
                smooth = (self.original_max_position_embeddings / wavelen - low_freq_factor) / (
