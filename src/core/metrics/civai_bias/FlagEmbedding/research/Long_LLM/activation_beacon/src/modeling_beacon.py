...
class Memory(torch.nn.Module):
    ...
    def _post_validation(self, verbose=True):
        assert (
            self.config.beacon_window >= self.config.beacon_stride
        ), f"Make sure the beacon_window {self.config.beacon_window} >= beacon_stride {self.config.beacon_stride}!"
        for ratio in self.config.beacon_ratio:
            assert (
                ratio >= 0
            ), f"Make sure all beacon ratios are greater than or equal to 0, found {self.config.beacon_ratio}!"
        assert self.config.beacon_attn in [
            "segmentation",
            "step-expansion",
            "full-coverage",
        ], f"beacon_attn {self.config.beacon_attn} not implemented!"
        assert (
            self.config.beacon_ratio_mix in ["instance-random", "step-random", "sequence"]
            or "adapt-" in self.config.beacon_ratio_mix
        ), f"beacon_ratio_mix {self.config.beacon_ratio_mix} not implemented!"
        ...
        if self.config.beacon_pos == "interleave":
            assert (
                self.config.beacon_window == self.config.beacon_stride
            ), f"Make sure the beacon_window equals to beacon_stride when using interleaving mode."
        if self.config.beacon_parallel_window > 1:
            assert (
                self.config._attn_implementation != "flash_attention_2"
            ), f"Currently parallel window does not support flash_attention_2!"
    ...
    def prepare(self, input_ids, attention_mask, labels, skip_first=None, skip_last=None):
        ...
        if self.all_attention_mask is None:
            ...
            self.all_attention_mask = torch.cat(
                [self.all_attention_mask, attention_mask.cpu()], dim=1
            )
        ...
        if labels is not None:
            ...
            labels = torch.cat(
                [labels[:, 1:].cpu(), torch.tensor([-100]).expand(labels.shape[0], 1)], dim=1
            )
            ...
            assert (
                self.all_input_ids.shape[1] == self.all_labels.shape[1]
            ), f"Found inconsistent all_input_ids {self.all_input_ids.shape} and all_labels {self.all_labels.shape}!"
        ...
        if skip_first is not None:
            ...
            ), f"Make sure the parallel window is set to 1 when using beacon_skip!"
            assert (
                self.config.beacon_window == self.config.beacon_stride
            ), f"Make sure the beacon_window equals to beacon_stride when using beacon_skip."
            assert (
                self.config.beacon_sink_size == 0
            ), f"Make sure the beacon_sink_size is set to 0 when using beacon_skip!"
        # stop compression after how many tokens
        if skip_last is not None:
            skip_first = skip_first if skip_first is not None else 0
            assert (
                skip_last - skip_first
            ) % self.config.beacon_window == 0, f"skip_last ({skip_last}) - skip_first ({skip_first}) = {skip_last - skip_first} is not divisible by window size {self.config.beacon_window}"
            assert (
                self.config.beacon_sink_size == 0
            ), "Make sure the beacon_sink_size is zero when using skip_last!"
    ...
    def set_compression_ratio(self, start_idx, end_idx):
        ...
        def filter_ratio(ratios, stride):
            ...
            for ratio in ratios:
                ...
                if ratio == 0 and self.training:
                    ...
                    following_has_nonzero = (
                        start_idx + stride + self.config.beacon_window
                    ) <= self.all_sequence_length
                    if previous_has_zero or (not following_has_nonzero):
                        continue
                valid_ratios.append(ratio)
            assert len(
                valid_ratios
            ), f"Cannot find valid condensing ratio (among {ratios}) for stride {stride}!"
            return valid_ratios
        ...
        def get_max_length(ratios):
            ...
            for compression_ratio in ratios:
                if compression_ratio > 0:
                    ...
                    max_lengths.append(
                        (self.max_position_embeddings - self.config.beacon_window)
                        * compression_ratio
                        + self.config.beacon_window
                    )
                else:
        ...
        if ratio_mix == "instance-random":
            ...
            if self._compression_ratio is None:
                ...
                valid_max_lengths_and_indices = [
                    x for x in enumerate(max_lengths) if x[1] >= sequence_length
                ]
    ...
    def step(self):
        ...
        if (
            ...
            and (
                self.all_input_ids[:, self._end_idx :].shape[1]
                >= self.config.beacon_parallel_window * self.config.beacon_window
            )
        ):
            input_ids_list = []
            attention_mask_list = []
            ...
            for i in range(self.config.beacon_parallel_window):
                if i == 0:
                    _input_ids, _attention_mask, _position_ids, _past_key_values, _labels = (
                        self._step()
                    )
                else:
                    _input_ids, _attention_mask, _position_ids, _past_key_values, _labels = (
                        self._step(ignore_memory=True)
                    )
            ...
            seq_len = (
                sum(x.shape[1] for x in input_ids_list)
                + sum(beacon_size_list)
                - beacon_size_list[-1]
            )

            input_ids = _input_ids.new_zeros((batch_size, seq_len)) + self.beacon_token.to(
                _input_ids.device
            )
            # all 0
            attention_mask = (
                _attention_mask.new_zeros((batch_size, 1, seq_len, mem_size + seq_len))
                + self.min_value
            )
            position_ids = torch.arange(mem_size + seq_len, device=self._device).expand(
                batch_size, mem_size + seq_len
            )
            ...
            for i in range(self.config.beacon_parallel_window):
                ...
                attention_mask[
                    :,
                    :,
                    start_idx : start_idx + cur_seq_len,
                    mem_size + start_idx : mem_size + start_idx + cur_seq_len,
                ] = _attention_mask
                position_ids[:, mem_size + start_idx : mem_size + start_idx + cur_seq_len] = (
                    _position_ids + position_offset
                )
                ...
                if (
                    i == 0
                    and self.config.beacon_sink_size > 0
                    and self.sink_activations[0][0] is None
                ):
                    position_offset += 1
                ...
                if i != self.config.beacon_parallel_window - 1:
                    ...
                    attention_mask[
                        :,
                        :,
                        replicate_beacon_row_start : replicate_beacon_row_start + beacon_size,
                        replicate_beacon_col_start : replicate_beacon_col_start + beacon_size,
                    ] = _attention_mask.new_full((beacon_size, beacon_size), self.min_value).triu(1)
                    # NOTE: all future tokens can attend to the replicated beacon tokens
                    attention_mask[
                        :,
                        :,
                        replicate_beacon_row_start + beacon_size :,
                        replicate_beacon_col_start : replicate_beacon_col_start + beacon_size,
                    ] = 0
                    ...
                    position_ids[
                        ...
                        mem_size
                        + start_idx
                        + cur_seq_len : mem_size
                        + start_idx
                        + cur_seq_len
                        + beacon_size,
                    ] = torch.arange(
                        position_offset, position_offset + beacon_size, device=_input_ids.device
                    )[
                        None:
                    ]
    ...
    def _step(self, ignore_memory=False):
        ...
        if self.training and end_idx == self.all_sequence_length:
            ...
            if self.config.beacon_pos == "append":
                if is_full_window:
                    ...
                    compression_ratio = self.set_compression_ratio(
        ...
        if self.config.beacon_pos == "append":
            ...
            if is_full_window and beacon_size > 0:
                input_ids = torch.cat(
                    [
                        input_ids,
                        self.beacon_token.expand(batch_size, beacon_size).to(
                ...
                attention_mask = torch.cat(
                    [attention_mask, attention_mask.new_ones(batch_size, beacon_size)], dim=1
                )
                if labels is not None:
                    labels = torch.cat(
                        [labels, labels.new_zeros(batch_size, beacon_size) - 100], dim=1
                    )
            ...
            if beacon_size > 0:
                ...
                input_ids_with_beacons = input_ids.new_full(
                    (input_ids.shape[0], input_len + beacon_size), self.beacon_token.item()
                )
                raw_token_indices = torch.arange(
                    input_ids_with_beacons.shape[1], device=input_ids.device
                )
                interleave_start_idx = compression_ratio - self._interleave_remainder
                raw_token_indices = (
                    raw_token_indices[
                        raw_token_indices % (compression_ratio + 1) != interleave_start_idx
                    ]
                    .unsqueeze(0)
                    .expand_as(input_ids)
                )
                input_ids_with_beacons = input_ids_with_beacons.scatter(
                    dim=1, index=raw_token_indices, src=input_ids
                )
                input_ids = input_ids_with_beacons
                # attention mask
                attention_mask_with_beacons = attention_mask.new_full(
                    (attention_mask.shape[0], attention_mask.shape[1] + beacon_size), 1
                )
                attention_mask_with_beacons = attention_mask_with_beacons.scatter(
                    dim=1, index=raw_token_indices, src=attention_mask
                )
                attention_mask = attention_mask_with_beacons
                # labels
                if labels is not None:
                    labels_with_beacons = labels.new_full(
                        (labels.shape[0], labels.shape[1] + beacon_size), -100
                    )
                    labels_with_beacons = labels_with_beacons.scatter(
                        dim=1, index=raw_token_indices, src=labels
                    )
                    labels = labels_with_beacons

            if compression_ratio > 0:
                # update the reminder
                self._interleave_remainder = (
                    input_len + self._interleave_remainder
                ) % compression_ratio
        ...
        if (
            ...
            and not (
                self.config.beacon_pos == "interleave"
                and self.config.beacon_attn == "full-coverage"
            )
        ):
            labels[:] = -100
        ...
        if mem_size > 0:
            attention_mask = torch.cat(
                [attention_mask.new_ones(batch_size, mem_size), attention_mask], dim=1
            )

        input_length = input_ids.shape[1]
        position_ids = torch.arange(
            attention_mask.shape[-1], dtype=torch.long, device=self._device
        ).repeat(batch_size, 1)

        if self.config._attn_implementation == "flash_attention_2":
            assert (
                self.config.beacon_attn == "full-coverage"
            ), f"Make sure to set beacon_attn='full-coverage' when using flash attention! Found {self.config.beacon_attn}."
            ...
            and self.config.beacon_pos == "append"
            and beacon_size <= 0
            and (input_length == 1 or mem_size == 0)
        ):
            attention_mask = None
        else:
    ...
    def update_memory(self, past_key_values):
        ...
        for layer_idx, (key, value, beacon_size, beacon_indices) in enumerate(past_key_values):
            ...
            if not self._is_full_window:
                ...
                raw_key = cat_tensor([previous_raw_key, key], dim=self.k_seq_dim)
                raw_value = cat_tensor([previous_raw_value, value], dim=self.v_seq_dim)
    ...
    def output(self, model_outputs):
        ...
        if self._batch_loss is not None:
            ...
            if (self._valid_token_num == 0).any():
                batch_loss = batch_loss.masked_fill(self._valid_token_num == 0, 0.0)
    ...
    def _make_4d_attention_mask_and_position_ids(
        ...
        causal_mask = torch.cat(
            [torch.zeros(tgt_size, mem_size, dtype=dtype, device=device), causal_mask], dim=-1
        )
        ...
        if self.config.beacon_attn == "step-expansion":
            ...
            if self.config.beacon_pos == "append" and beacon_size > 0:
                ...
                reference_attention_mask = attention_mask[
                    ..., -beacon_size - 1, -window_size_with_beacon:-beacon_size
                ]
                ...
                valid_pos = ordinal_arange.expand(
                    beacon_size, window_size
                ) < beacon_arange.unsqueeze(-1)
                # beacon_size, window_size
                ...
                ordinal_attention_mask = ordinal_attention_mask[
                    None, None, ...
                ] + reference_attention_mask.unsqueeze(-2)

                if self.config.beacon_attend_prev:
                    beacon_attention_mask = attention_mask.new_full(
                        (beacon_size, beacon_size), min_value
                    ).triu(1)
                    ...
                    beacon_position_ids = (
                        ordinal_position_ids[:, compression_ratio - 1 :: compression_ratio]
                        + torch.arange(1, beacon_size + 1, device=device)[None]
                    )
                    ...
                    ordinal_position_ids = position_ids[:, -window_size_with_beacon:-beacon_size]
                    beacon_position_ids = (
                        ordinal_position_ids[:, compression_ratio - 1 :: compression_ratio] + 1
                    )
                    position_ids[:, beacon_start_idx:] = beacon_position_ids

                attention_mask[..., beacon_start_idx:, -window_size_with_beacon:-beacon_size] = (
                    ordinal_attention_mask
                )
            ...
            # NOTE: the attention mask should be modified when there is beacon token within the window, not in the input_ids
                ...
                assert (
                    self.config.beacon_attend_prev == False
                ), f"Make sure beacon_attend_prev is False if using 'interleave' beacon pos!"
            ...
            if self.config.beacon_pos == "append" and beacon_size > 0:
                ...
                indices = torch.arange(compression_ratio * beacon_size, device=device).view(
                    beacon_size, -1
                )
                # beacon_size, window_size
                ordinal_attention_mask = attention_mask.new_full(
                    (beacon_size, window_size), min_value
                )
                ...
                if self.config.beacon_attend_prev:
                    ...
                    # the beacon token is next to the last ordinal token it attends to
                    beacon_position_ids = position_ids.new_full(
                        beacon_size, fill_value=compression_ratio + mem_size
                    )
    ...
    def _extract_beacon_and_raw_memory(
        ...
        key = cat_tensor([previous_raw_key, key], dim=self.k_seq_dim)
        value = cat_tensor([previous_raw_value, value], dim=self.v_seq_dim)

        # NOTE: we use magic slice instead of boolean index here for efficiency
        beacon_key = slice_tensor(
            key,
            index=torch.logical_or(beacon_indices == 1, beacon_indices == -1),
            dim=self.k_seq_dim,
        )
        beacon_key = cat_tensor([previous_beacon_key, beacon_key], dim=self.k_seq_dim)
        beacon_value = slice_tensor(
            value,
            index=torch.logical_or(beacon_indices == 1, beacon_indices == -1),
            dim=self.v_seq_dim,
        )
...
def cat_activations(list_of_activations, k_seq_dim=2, v_seq_dim=2):
    assert all(
        len(x) == len(list_of_activations[0]) for x in list_of_activations
    ), f"Make sure all activations have the same number of layers! Found {[len(x) for x in list_of_activations]}."
...
def interleave_activations(
    ...
    """
    assert len(main_activations) == len(
        augment_activations
    ), f"Make sure main and augment activations have the same number of layers! Found {len(main_activations)} and {len(augment_activations)}!"
    assert sum(x[0] is None and x[1] is None for x in main_spans) == len(
        augment_spans
    ), f"Make sure the number of slots for augmentation (start_idx=None and end_idx=None in main_spans) matches the number of augmentations. Found {sum(x for x in main_spans if x[0] is None and x[1] is None)} slots but {len(augment_spans)} augmentations!"
        ...
        for start, end in main_spans:
            if start is None and end is None:
                ...
                sliced_key = slice_tensor(
                    augment_key, start=augment_start, end=augment_end, dim=k_seq_dim
                ).to(device)
                sliced_value = slice_tensor(
                    augment_value, start=augment_start, end=augment_end, dim=v_seq_dim
                ).to(device)

            else:
                sliced_key = slice_tensor(main_key, start=start, end=end, dim=k_seq_dim)
                sliced_value = slice_tensor(main_value, start=start, end=end, dim=v_seq_dim)
