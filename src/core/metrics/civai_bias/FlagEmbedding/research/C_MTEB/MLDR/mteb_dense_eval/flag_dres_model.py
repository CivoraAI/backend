    ...
    def encode_queries(self, queries: List[str], **kwargs) -> np.ndarray:
        ...
        if isinstance(queries[0], dict):
            if self.query_instruction_for_retrieval is not None:
                input_texts = [
                    "{}{}".format(self.query_instruction_for_retrieval, q["text"]) for q in queries
                ]
            else:
            ...
            if self.query_instruction_for_retrieval is not None:
                input_texts = [
                    "{}{}".format(self.query_instruction_for_retrieval, q) for q in queries
                ]
            else:
                input_texts = queries
        return self.encode(
    ...
    def encode_corpus(self, corpus: List[Union[Dict[str, str], str]], **kwargs) -> np.ndarray:
        ...
        if isinstance(corpus[0], dict):
            if self.passage_instruction_for_retrieval is not None:
                input_texts = [
                    "{}{} {}".format(
                        self.passage_instruction_for_retrieval, doc.get("title", ""), doc["text"]
                    ).strip()
                    for doc in corpus
                ]
            else:
            ...
            if self.passage_instruction_for_retrieval is not None:
                ...
                input_texts = corpus
        return self.encode(
            input_texts, max_length=self.max_passage_length, batch_size=self.corpus_batch_size
        )

    @torch.no_grad()
    def encode(
        self, sentences: List[str], max_length: int, batch_size: int, **kwargs
    ) -> np.ndarray:
        if self.num_gpus > 0:
        ...
        if self.pooling_method == "last":
            dataset.set_transform(
                partial(_transform_func_v2, tokenizer=self.tokenizer, max_length=max_length)
            )
        else:
            dataset.set_transform(
                partial(_transform_func, tokenizer=self.tokenizer, max_length=max_length)
            )
