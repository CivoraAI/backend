...
def _map_func_corpus(examples):
    ...
    inputs = tokenizer(examples["text"], padding=False, truncation=True, max_length=8192)
...
def _map_func_query(examples):
    ...
    inputs = tokenizer(examples["query"], padding=False, truncation=True, max_length=512)
...
def generate_corpus(lang: str, corpus_save_dir: str):
    ...
    corpus_list = [
        {"id": e["docid"], "contents": e["text"]} for e in tqdm(corpus, desc="Generating corpus")
    ]
