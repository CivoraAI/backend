...
class EvalArgs:
    ...
    save_dir: str = field(
        ...
        metadata={
            "help": "Dir to save encoded query and corpus. Encoded query and corpus will be saved to `save_dir/{encoder_name}/{lang}/query_embd.tsv` and `save_dir/{encoder_name}/{lang}/corpus/corpus_embd.jsonl`, individually."
        },
    )
    ...
    corpus_batch_size: int = field(default=4, metadata={"help": "Inference batch size."})
...
def load_corpus(lang: str):
    ...
    corpus_list = [
        {"id": e["docid"], "content": e["text"]} for e in tqdm(corpus, desc="Generating corpus")
    ]
...
def get_queries(lang: str, split: str = "test"):
    ...
    queries_list = []
    for data in dataset:
        queries_list.append({"id": data["query_id"], "content": data["query"]})
...
def main():
    ...
    for lang in languages:
        ...
        if (
            os.path.exists(os.path.join(save_dir, "corpus", "corpus_embd.jsonl"))
