...
@dataclass
class EvalArgs:
    results_save_path: str = field(default="./results", metadata={"help": "Path to save results."})
    languages: str = field(
        default=None,
        metadata={
...
@dataclass
class ModelArgs:
    encoder: str = field(default="BAAI/bge-m3", metadata={"help": "encoder name or path."})
    pooling_method: str = field(
        default="cls", metadata={"help": "Pooling method. Avaliable methods: 'cls', 'mean', 'last'"}
    )
...
def main():
    ...
    model = FlagDRESModel(
        ...
        query_instruction_for_retrieval=(
            model_args.query_instruction_for_retrieval if model_args.add_instruction else None
        ),
        passage_instruction_for_retrieval=(
            model_args.passage_instruction_for_retrieval if model_args.add_instruction else None
        ),
    ...
    output_folder = os.path.join(
        eval_args.results_save_path,
        f"{os.path.basename(encoder)}_max-length-{model_args.max_passage_length}",
    )
    ...
    evaluation = MTEB(tasks=[MultiLongDocRetrieval(langs=languages)])
    results_dict = evaluation.run(
        model,
        eval_splits=["test"],
        output_folder=output_folder,
        overwrite_results=eval_args.overwrite,
