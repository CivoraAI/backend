...
class EvalArgs:
    model_name_or_path: str = field(
        default="BAAI/bge-m3", metadata={"help": "Name or path of model"}
    )
    languages: str = field(
        default="en",
        metadata={
    ...
    top_k: int = field(
        default=1000, metadata={"help": "Use reranker to rerank top-k retrieval results"}
    )
    sparse_weight: float = field(default=0.8, metadata={"help": "Hybrid weight of sparse score"})
    dense_weight: float = field(default=0.2, metadata={"help": "Hybrid weight of dense score"})
    dense_search_result_save_dir: str = field(
        default="../dense_retrieval/search_results",
        metadata={
            "help": "Dir to saving dense search results. Search results path is `dense_search_result_save_dir/{model_name_or_path}/{lang}.txt`"
        },
    )
    sparse_search_result_save_dir: str = field(
        default="../sparse_retrieval/search_results",
        metadata={
            "help": "Dir to saving sparse search results. Search results path is `sparse_search_result_save_dir/{model_name_or_path}/{lang}.txt`"
        },
    )
    hybrid_result_save_dir: str = field(
        default="./search_results",
        metadata={
            "help": "Dir to saving hybrid search results. Reranked results will be saved to `hybrid_result_save_dir/{model_name_or_path}/{lang}.txt`"
        },
    )
...
def main():
    ...
    if os.path.basename(eval_args.model_name_or_path).startswith("checkpoint-"):
        eval_args.model_name_or_path = (
            os.path.dirname(eval_args.model_name_or_path)
            + "_"
            + os.path.basename(eval_args.model_name_or_path)
        )
    ...
    for lang in languages:
        ...
        hybrid_result_save_path = os.path.join(
            eval_args.hybrid_result_save_dir,
            f"{os.path.basename(eval_args.model_name_or_path)}",
            f"{lang}.txt",
        )

        sparse_search_result_save_dir = os.path.join(
            eval_args.sparse_search_result_save_dir, os.path.basename(eval_args.model_name_or_path)
        )
        ...
        dense_search_result_save_dir = os.path.join(
            eval_args.dense_search_result_save_dir, os.path.basename(eval_args.model_name_or_path)
        )
