...
class EvalArgs:
    ...
    save_dir: str = field(
        ...
        metadata={
            "help": "Dir to save encoded query and corpus. Encoded query and corpus will be saved to `save_dir/{encoder_name}/{lang}/query_embd.tsv` and `save_dir/{encoder_name}/corpus/corpus_embd.jsonl`, individually."
        },
    )
...
def get_queries(qa_data_dir: str, lang: str):
    ...
    for data in dataset:
        _id = str(data["id"])
        query = data["question"]
        queries_list.append({"id": _id, "content": query})

    queries = datasets.Dataset.from_list(queries_list)
    return queries


def encode_and_save_corpus(
    corpus_save_path: str,
    model: BGEM3FlagModel,
...
def main():
    ...
    if (
        os.path.exists(corpus_save_path)
        and os.path.getsize(corpus_save_path) > 0
