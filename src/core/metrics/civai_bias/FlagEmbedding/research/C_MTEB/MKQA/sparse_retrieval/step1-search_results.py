...
class EvalArgs:
    ...
    encoded_query_and_corpus_save_dir: str = field(
        ...
        metadata={
            "help": "Dir to save encoded queries and corpus. Encoded queries and corpus are saved in `save_dir/{encoder_name}/{lang}/query_embd.tsv` and `save_dir/{encoder_name}/corpus/corpus_embd.jsonl`, individually."
        },
    )
    result_save_dir: str = field(
        default="./search_results",
        metadata={
            "help": "Dir to saving results. Search results will be saved to `result_save_dir/{encoder_name}/{lang}.txt`"
        },
    )
    qa_data_dir: str = field(default="../qa_data", metadata={"help": "Dir to qa data."})
    batch_size: int = field(default=32, metadata={"help": "Batch size to use during search"})
    threads: int = field(default=1, metadata={"help": "Maximum threads to use during search"})
    hits: int = field(default=1000, metadata={"help": "Number of hits"})
...
def main():
    ...
    corpus_embd_dir = os.path.join(
        eval_args.encoded_query_and_corpus_save_dir, os.path.basename(encoder), "corpus"
    )
    index_save_dir = os.path.join(
        eval_args.encoded_query_and_corpus_save_dir, os.path.basename(encoder), "index"
    )
    ...
    for lang in languages:
        ...
        result_save_path = os.path.join(
            eval_args.result_save_dir, os.path.basename(encoder), f"{lang}.txt"
        )
        ...
        encoded_query_and_corpus_save_dir = os.path.join(
            eval_args.encoded_query_and_corpus_save_dir, os.path.basename(encoder), lang
        )
