...
class EvalArgs:
    ...
    search_result_save_dir: str = field(
        ...
        metadata={
            "help": "Dir to saving search results. Search results path is `result_save_dir/{encoder}-{reranker}/{lang}.txt`"
        },
    )
    qa_data_dir: str = field(default="../qa_data", metadata={"help": "Dir to qa data."})
    metrics: str = field(
        default="recall@20",
        metadata={"help": "Metrics to evaluate. Avaliable metrics: recall@k", "nargs": "+"},
    )
    eval_result_save_dir: str = field(
        default="./reranker_evaluation_results",
        metadata={
            "help": "Dir to saving evaluation results. Evaluation results will be saved to `eval_result_save_dir/{encoder}-{reranker}.json`"
        },
    )
    threads: int = field(
...
def save_results(
    model_name: str, reranker_name: str, results: dict, save_path: str, eval_languages: list
):
    try:
    ...
    results_dict = {"reranker": reranker_name, "model": model_name, "results": results}
    with open(save_path, "w", encoding="utf-8") as f:
        json.dump(results_dict, f, indent=4, ensure_ascii=False)
    print(
        f"Results of evaluating `{reranker_name}` on `{eval_languages}` based on `{model_name}` saved at `{save_path}`"
    )
...
def get_search_result_dict(search_result_path: str, top_k: int = 100):
    ...
    flag = True
    for _, row in tqdm(
        pd.read_csv(search_result_path, sep=" ", header=None).iterrows(),
        desc="Loading search results",
    ):
        qid = str(row.iloc[0])
    ...
    try:
        for sub_dir in ["colbert", "sparse", "dense", "colbert+sparse+dense"]:
            ...
            if eval_args.threads > 1:
                ...
                for lang in languages:
                    ...
                    search_result_save_dir = os.path.join(
                        eval_args.search_result_save_dir,
                        sub_dir,
                        f"{os.path.basename(eval_args.encoder)}-{os.path.basename(eval_args.reranker)}",
                    )
                    ...
                    results_list.append(
                        pool.apply_async(
                            evaluate,
                            args=(corpus_dict, qa_dict, search_result_path, eval_args.metrics),
                        )
                    )
                pool.close()
                pool.join()
            ...
            save_results(
                ...
                save_path=os.path.join(
                    eval_args.eval_result_save_dir,
                    sub_dir,
                    f"{os.path.basename(eval_args.encoder)}-{os.path.basename(eval_args.reranker)}.json",
                ),
        ...
        if eval_args.threads > 1:
            ...
            for lang in languages:
                ...
                results_list.append(
                    pool.apply_async(
                        evaluate, args=(corpus_dict, qa_dict, search_result_path, eval_args.metrics)
                    )
                )
            pool.close()
            pool.join()
