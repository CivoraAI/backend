...
@dataclass
class EvalArgs:
    encoder: str = field(default="BAAI/bge-m3", metadata={"help": "Name or path of model"})
    reranker: str = field(default="BAAI/bge-m3", metadata={"help": "Name or path of reranker"})
    languages: str = field(
        default="en",
        metadata={
            "help": "Languages to evaluate. Avaliable languages: en ar fi ja ko ru es sv he th da de fr it nl pl pt hu vi ms km no tr zh_cn zh_hk zh_tw",
            "nargs": "+",
        },
    )
    top_k: int = field(
        default=200, metadata={"help": "Use reranker to rerank top-k retrieval results"}
    )
    dense_weight: float = field(default=1, metadata={"help": "Hybrid weight of sparse score"})
    sparse_weight: float = field(default=0.3, metadata={"help": "Hybrid weight of sparse score"})
    colbert_weight: float = field(default=1, metadata={"help": "Hybrid weight of sparse score"})
    dense_search_result_save_dir: str = field(
        default="../rerank/unify_rerank_results/dense",
        metadata={
            "help": "Dir to saving dense search results. Search results path is `dense_search_result_save_dir/{encoder}-{reranker}/{lang}.txt`"
        },
    )
    sparse_search_result_save_dir: str = field(
        default="../rerank/unify_rerank_results/sparse",
        metadata={
            "help": "Dir to saving sparse search results. Search results path is `sparse_search_result_save_dir/{encoder}-{reranker}/{lang}.txt`"
        },
    )
    ...
    hybrid_result_save_dir: str = field(
        ...
        metadata={
            "help": "Dir to saving hybrid search results. Reranked results will be saved to `hybrid_result_save_dir/{encoder}-{reranker}/{lang}.txt`"
        },
    )
    threads: int = field(
        default=1, metadata={"help": "num of evaluation threads. <= 1 means single thread"}
    )
...
def check_languages(languages):
    ...
    for lang in languages:
        if lang not in avaliable_languages:
            raise ValueError(
                f"Language `{lang}` is not supported. Avaliable languages: {avaliable_languages}"
            )
    return languages
...
def save_hybrid_results(
    sparse_search_result_dict: dict,
    dense_search_result_dict: dict,
    colbert_search_result_dict: dict,
    hybrid_result_save_path: str,
    top_k: int = 1000,
    ...
    qid_list = list(
        set(sparse_search_result_dict.keys())
        | set(dense_search_result_dict.keys())
        | set(colbert_search_result_dict.keys())
    )
    hybrid_results_list = []
    ...
    with open(hybrid_result_save_path, "w", encoding="utf-8") as f:
        for qid, hybrid_results in tqdm(
            zip(qid_list, hybrid_results_list), desc="Saving hybrid search results"
        ):
...
def main():
    ...
    if os.path.basename(eval_args.encoder).startswith("checkpoint-"):
        eval_args.encoder = (
            os.path.dirname(eval_args.encoder) + "_" + os.path.basename(eval_args.encoder)
        )

    if os.path.basename(eval_args.reranker).startswith("checkpoint-"):
        eval_args.reranker = (
            os.path.dirname(eval_args.reranker) + "_" + os.path.basename(eval_args.reranker)
        )
    ...
    if eval_args.threads > 1:
        ...
        for lang in languages:
            ...
            hybrid_result_save_path = os.path.join(
                eval_args.hybrid_result_save_dir, dir_name, f"{lang}.txt"
            )

            sparse_search_result_save_dir = os.path.join(
                eval_args.sparse_search_result_save_dir, dir_name
            )

            sparse_search_result_path = os.path.join(sparse_search_result_save_dir, f"{lang}.txt")

            sparse_search_result_dict = get_search_result_dict(
                sparse_search_result_path, top_k=eval_args.top_k
            )

            dense_search_result_save_dir = os.path.join(
                eval_args.dense_search_result_save_dir, dir_name
            )

            dense_search_result_path = os.path.join(dense_search_result_save_dir, f"{lang}.txt")

            dense_search_result_dict = get_search_result_dict(
                dense_search_result_path, top_k=eval_args.top_k
            )

            colbert_search_result_save_dir = os.path.join(
                eval_args.colbert_search_result_save_dir, dir_name
            )

            colbert_search_result_path = os.path.join(colbert_search_result_save_dir, f"{lang}.txt")

            colbert_search_result_dict = get_search_result_dict(
                colbert_search_result_path, top_k=eval_args.top_k
            )
