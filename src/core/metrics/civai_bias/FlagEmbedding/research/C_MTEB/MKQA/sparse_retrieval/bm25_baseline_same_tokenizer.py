...
def _map_func_corpus(examples):
    ...
    inputs = tokenizer(examples["contents"], padding=False, truncation=True, max_length=512)
...
def _map_func_query(examples):
    ...
    inputs = tokenizer(examples["question"], padding=False, truncation=True, max_length=512)
    input_ids_list = inputs["input_ids"]
    for i in range(len(examples["id"])):
        token_ids = input_ids_list[i][1:-1]
        token_ids = [str(_id) for _id in token_ids]
