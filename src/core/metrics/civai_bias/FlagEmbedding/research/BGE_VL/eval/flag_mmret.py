...
class Flag_mmret(nn.Module):
    ...
    def encode_corpus(
        ...
        if corpus_type == "text":
            ...
            return self.encode_mm_it(
                corpus["text"], corpus["image"], batch_size=batch_size, max_length=max_length
            )
    ...
    @torch.no_grad()
    def encode_text(
        self, sentences: Union[List[str], str], batch_size: int = 256, max_length: int = 77
    ) -> np.ndarray:
        if self.num_gpus > 0:
    ...
    def encode_mm_it(
        self,
        captions: Union[List[str], str],
        image_ids: Union[List[str], str],
        batch_size: int = 256,
        max_length: int = 77,
    ) -> np.ndarray:
        if self.num_gpus > 0:
        ...
        mm_it_dataloader = DataLoader(
            dataset=mm_it_dataset,
        ...
        for data in tqdm(
            mm_it_dataloader, desc="Inference Embeddings", disable=len(captions) < 256
        ):
    ...
    @torch.no_grad()
    def encode_image(
        self, image_ids: Union[List[str], str], batch_size: int = 256, max_length: int = 77
    ) -> np.ndarray:
        if self.num_gpus > 0:
            batch_size = batch_size * self.num_gpus
        self.model.eval()
        ...
        image_dataset = Image_Dataset(
            image_ids=image_ids, image_dir=self.image_dir, image_processor=self.image_processor
        )
