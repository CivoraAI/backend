...
@dataclass
class Args:
    model_name: str = field(default="BAAI/BGE-VL-large", metadata={"help": "Model Name"})
    image_dir: str = field(
    ...
    max_query_length: int = field(default=64, metadata={"help": "Max query length."})
    max_passage_length: int = field(default=77, metadata={"help": "Max passage length."})
    batch_size: int = field(default=256, metadata={"help": "Inference batch size."})
    index_factory: str = field(default="Flat", metadata={"help": "Faiss index factory."})
    ...
    load_embedding: bool = field(default=False, metadata={"help": "Load embeddings from save_dir?"})
    save_path: str = field(
...
def index(
    ...
    if load_embedding:
        ...
        corpus_embeddings = model.encode_corpus(
            corpus, batch_size=batch_size, max_length=max_length, corpus_type="image"
        )
        ...
        if save_embedding:
            ...
            memmap = np.memmap(
                save_path, shape=corpus_embeddings.shape, mode="w+", dtype=corpus_embeddings.dtype
            )
...
def main():
    ...
    eval_data = datasets.load_dataset(
        "json", data_files="./eval/data/fashioniq_shirt_query_val.jsonl", split="train"
    )
    image_corpus = datasets.load_dataset(
        "json", data_files="./eval/data/fashioniq_shirt_corpus.jsonl", split="train"
    )
    ...
    eval_data = datasets.load_dataset(
        "json", data_files="./eval/data/fashioniq_dress_query_val.jsonl", split="train"
    )
    image_corpus = datasets.load_dataset(
        "json", data_files="./eval/data/fashioniq_dress_corpus.jsonl", split="train"
    )
    ...
    eval_data = datasets.load_dataset(
        "json", data_files="./eval/data/fashioniq_toptee_query_val.jsonl", split="train"
    )
    image_corpus = datasets.load_dataset(
        "json", data_files="./eval/data/fashioniq_toptee_corpus.jsonl", split="train"
    )
    ...
    print(
        f"toptee: {metrics_toptee['Recall@10'] * 100:.2f} / {metrics_toptee['Recall@50'] * 100:.2f}"
    )
    print(
        f"overall: {(metrics_shirt['Recall@10'] + metrics_dress['Recall@10'] + metrics_toptee['Recall@10']) * 100 / 3:.2f} / {(metrics_shirt['Recall@50'] + metrics_dress['Recall@50'] + metrics_toptee['Recall@50']) * 100 / 3:.2f}"
    )
