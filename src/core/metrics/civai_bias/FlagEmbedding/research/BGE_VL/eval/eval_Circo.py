...
class Args:
    ...
    result_save_path: str = field(
        default="./eval/mmret_large_circo.json", metadata={"help": "Where to save the results."}
    )
    image_dir: str = field(
        default="YOUR_COCO_IMAGE_DIRECTORY", metadata={"help": "Where the images located on."}
    )
...
def main():
    ...
    eval_data = datasets.load_dataset(
        "json", data_files="./eval/data/circo_query.jsonl", split="train"
    )
    image_corpus_test = datasets.load_dataset(
        "json", data_files="./eval/data/circo_corpus.jsonl", split="train"
    )
