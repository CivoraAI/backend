...
class AbsEvalArgs:
    ...
    force_redownload: bool = field(
        default=False,
        metadata={
            "help": "Whether to force redownload the dataset. This is useful when you load dataset from remote and want to update the dataset."
        },
    )
    ...
    corpus_embd_save_dir: str = field(
        default=None,
        metadata={"help": "Path to save corpus embeddings. If None, embeddings are not saved."},
    )
    output_dir: str = field(default="./search_results", metadata={"help": "Path to save results."})
    search_top_k: int = field(default=1000, metadata={"help": "Top k for retrieving."})
    ...
    k_values: int = field(
        ...
        metadata={
            "help": "k values for evaluation. Default: [1, 3, 5, 10, 100, 1000]",
            "nargs": "+",
        },
    )
    eval_output_method: str = field(
        default="markdown",
        metadata={
            "help": "The output method for evaluation results. Available methods: ['json', 'markdown']. Default: markdown.",
    ...
    eval_metrics: str = field(
        ...
        metadata={
            "help": "The metrics to evaluate. Default: ['ndcg_at_10', 'recall_at_10']",
            "nargs": "+",
        },
    )
...
class AbsEvalModelArgs:
    ...
    use_fp16: bool = field(default=True, metadata={"help": "whether to use fp16 for inference"})
    ...
    examples_for_task: Optional[str] = field(default=None, metadata={"help": "Examples for task"})
    ...
    reranker_model_class: Optional[str] = field(
        default=None,
        metadata={
            "help": "The reranker model class. Available classes: ['encoder-only-base', 'decoder-only-base', 'decoder-only-layerwise', 'decoder-only-lightweight']. Default: None. For the custom model, you need to specify the model class.",
            "choices": [
                "encoder-only-base",
                "decoder-only-base",
    ...
    use_bf16: bool = field(default=False, metadata={"help": "whether to use bf16 for inference"})
    ...
    cache_dir: str = field(default=None, metadata={"help": "Cache directory for models."})
    # ================ for inference ===============
    embedder_batch_size: int = field(default=3000, metadata={"help": "Batch size for inference."})
    reranker_batch_size: int = field(default=3000, metadata={"help": "Batch size for inference."})
    embedder_query_max_length: int = field(default=512, metadata={"help": "Max length for query."})
    ...
    reranker_max_length: int = field(default=512, metadata={"help": "Max length for reranking."})
    normalize: bool = field(
        default=False, metadata={"help": "whether to normalize the reranking scores"}
    )
    prompt: Optional[str] = field(default=None, metadata={"help": "The prompt for the reranker."})
    ...
    compress_layers: Optional[int] = field(
        default=None,
        metadata={"help": "The compress layers of lightweight reranker.", "nargs": "+"},
    )
    ...
    def __post_init__(self):
        ...
        if "\\n" in self.query_instruction_format_for_rerank:
            self.query_instruction_format_for_rerank = (
                self.query_instruction_format_for_rerank.replace("\\n", "\n")
            )
        if "\\n" in self.passage_instruction_format_for_rerank:
            self.passage_instruction_format_for_rerank = (
                self.passage_instruction_format_for_rerank.replace("\\n", "\n")
            )
