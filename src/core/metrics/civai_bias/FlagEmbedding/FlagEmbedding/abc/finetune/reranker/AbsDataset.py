...
class AbsRerankerTrainDataset(Dataset):
    ...
    def _shuffle_text(self, text):
        ...
        """
        if (
            self.args.shuffle_ratio > 0
            and len(text) > 100
            and random.random() < self.args.shuffle_ratio
        ):
            split_text = []
    ...
    def __getitem__(self, item):
        ...
        if self.args.query_instruction_for_rerank is not None:
            query = self.args.query_instruction_format.format(
                (
                    data["query_prompt"]
                    if "query_prompt" in data
                    else self.args.query_instruction_for_rerank
                ),
                query,
            )
        ...
        if self.args.passage_instruction_for_rerank is not None:
            passages = [
                self.args.passage_instruction_format.format(
                    (
                        data["passage_prompt"]
                        if "passage_prompt" in data
                        else self.args.passage_instruction_for_rerank
                    ),
                    p,
                )
                for p in passages
