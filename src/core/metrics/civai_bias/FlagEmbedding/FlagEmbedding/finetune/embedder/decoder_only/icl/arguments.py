...
class DecoderOnlyEmbedderICLModelArguments(AbsEmbedderModelArguments):
    ...
    use_flash_attn: bool = field(
        default=False, metadata={"help": "If passed, will use flash attention to train the model."}
    )
    ...
    additional_special_tokens: Optional[str] = field(
        default=None, metadata={"help": "additional special tokens", "nargs": "+"}
    )
...
class DecoderOnlyEmbedderICLDataArguments(AbsEmbedderDataArguments):
    ...
    example_query_max_len: int = field(
        default=64, metadata={"help": "The max length of example query."}
    )
    example_passage_max_len: int = field(
        default=96, metadata={"help": "The max length of example passage."}
    )
    retrieval_use_examples: bool = field(
        default=True, metadata={"help": "If passed, will use examples for retrieval."}
    )
