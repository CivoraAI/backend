...
class DecoderOnlyEmbedderICLSameDatasetTrainDataset(AbsEmbedderSameDatasetTrainDataset):
    ...
    def __init__(
        ...
        self.suffix = self.tokenizer(
            f"{self.args.icl_suffix_str}{self.tokenizer.eos_token}", add_special_tokens=False
        )["input_ids"]

        self.prefix = self.tokenizer(f"{self.tokenizer.bos_token}", add_special_tokens=False)[
            "input_ids"
        ]
        ...
        for i in range(len(queries)):
            ...
            if choices > 0 and len(icl_pairs) > 0:
                prefix_ids = random.sample(
                    list(range(len(icl_pairs))), min(choices + 1, len(icl_pairs))
                )
                if i in prefix_ids:
                    prefix_ids.remove(i)
