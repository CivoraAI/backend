...
class MTEBEvalRunner(AbsEvalRunner):
    ...
    def load_retriever_and_reranker(
        self,
    ) -> Tuple[MTEBEvalDenseRetriever, Union[MTEBEvalReranker, None]]:
        ...
        for task_type in tasks_results.keys():
            ...
            for task_name in tasks_results[task_type].keys():
                if "CQADupstack" in task_name:
                    ...
                    new_results[task_type][task_name] = float(
                        round(tasks_results[task_type][task_name], 2)
                    )
                    all_results_num += 1
    ...
    def run(self):
        ...
        tasks = mteb.get_tasks(languages=languages, tasks=tasks, task_types=task_types)
        ...
        for task in tasks:
            ...
            if self.eval_args.examples_path is not None:
                try:
                    eg_pairs = json.load(
                        open(os.path.join(self.eval_args.examples_path, task_name + ".json"))
                    )
                except:
            ...
            results = evaluation.run(
                self.retriever, output_folder=f"{output_folder}/{str(self.retriever)}"
            )

        ensure_dir(self.eval_args.eval_output_path)
        logger.info("Start computing metrics. Only save results as json.")
        tasks_results = self.read_results(
            f"{output_folder}/{str(self.retriever)}/no_model_name_available/no_revision_available",
            tasks,
        )
