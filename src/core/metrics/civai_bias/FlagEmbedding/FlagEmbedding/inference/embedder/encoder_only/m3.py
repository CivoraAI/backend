...
class M3Embedder(AbsEmbedder):
    ...
    def compute_lexical_matching_score(
        ...
        if isinstance(lexical_weights_1, dict) and isinstance(lexical_weights_2, dict):
            ...
            for lw1 in lexical_weights_1:
                scores_array.append(
                    [_compute_single_lexical_matching_score(lw1, lw2) for lw2 in lexical_weights_2]
                )
        ...
        def _process_token_weights(token_weights: np.ndarray, input_ids: list):
            ...
            for _token in ["cls_token", "eos_token", "pad_token", "unk_token"]:
                if _token in self.tokenizer.special_tokens_map:
                    _token_id = self.tokenizer.convert_tokens_to_ids(
                        self.tokenizer.special_tokens_map[_token]
                    )
    ...
    def compute_score(
        ...
        **kwargs: Any,
        ...
        Literal["colbert", "sparse", "dense", "sparse+dense", "colbert+sparse+dense"], List[float]
    ]:
    ...
    def compute_score_multi_process(
        ...
        **kwargs,
        ...
        scores_dict = self._concatenate_compute_score_results_from_multi_process(
    ...
    def compute_score_single_device(
        ...
        **kwargs: Any,
        ...
        for start_index in tqdm(
            ...
            q_dense_vecs, q_sparse_vecs, q_colbert_vecs = (
                queries_output["dense_vecs"],
                queries_output["sparse_vecs"],
                queries_output["colbert_vecs"],
            )
            p_dense_vecs, p_sparse_vecs, p_colbert_vecs = (
                corpus_output["dense_vecs"],
                corpus_output["sparse_vecs"],
            ...
            colbert_scores = self.model.compute_colbert_score(
                q_colbert_vecs, p_colbert_vecs, q_mask=queries_inputs["attention_mask"]
            )
            ...
            all_scores["colbert"].extend(colbert_scores.cpu().numpy().tolist())
            all_scores["sparse"].extend(sparse_scores.cpu().numpy().tolist())
            all_scores["dense"].extend(dense_scores.cpu().numpy().tolist())
    ...
    def _concatenate_results_from_multi_process(
        ...
        """
        merged_results = {"dense_vecs": [], "lexical_weights": [], "colbert_vecs": []}
