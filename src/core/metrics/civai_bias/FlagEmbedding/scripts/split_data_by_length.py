...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=".split_log",
        help="the name of log file, default: `.split_log`, which will be saved to `output_dir`",
    )
    parser.add_argument(
        "--length_list",
        type=int,
        ...
        nargs="+",
        help="the length list to split",
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default="BAAI/bge-m3",
        help="the model name or path of the tokenizer",
    )
    parser.add_argument(
        "--num_proc", type=int, default=16, help="the number of process, default: 16"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        default=False,
        help="whether to overwrite the output file, default: False",
    )
    ...
    def _process_file(self, file_path: str, output_path: str):
        ...
        try:
            dataset = load_dataset(
                "json", data_files=file_path, cache_dir=self.cache_dir, features=features
            )["train"]
        except:
            dataset = load_dataset(
                "json", data_files=file_path, cache_dir=self.cache_dir, features=kd_features
            )["train"]
        ...
        for length_l, length_r in self.length_ranges_list:
            ...
            idxs = mapped_dataset.filter(
                lambda x: length_l <= x["max_length"] < length_r, num_proc=self.num_proc
            )
...
def main(args):
    ...
    handler.run(input_path=input_path, output_dir=output_dir, log_name=log_name)
    print("\nDONE!")
